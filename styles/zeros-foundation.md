...
**Highlighted (brand accent — Orka Black family):**

```
--highlighted-bg              user-message bubble, some selected states
                              (text/icon on it: fg1/fg2)
--highlighted-bright          input focus, brighter accent borders, brand cue
```
...
8. **`--highlighted-bright` is the brand accent for focus / borders.** It is NOT a button fill.
...
## §9 — Light theme (shipped 2026-07-11)

The light theme lives in the `[data-theme="light"]` block of `zeros-tokens.css` — warm off-whites over the same token taxonomy. The appearance store writes `data-theme` on `<html>` (System / Light / Dark in Settings → Appearance; System follows macOS).

Light-specific shape to keep in mind when composing UI:
...
- The Electron window's pre-paint `backgroundColor` tracks the theme via the `window_set_background` IPC command (reported from `applyTheme`, persisted for the next launch).
