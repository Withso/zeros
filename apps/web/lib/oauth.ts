...
import { b64url, sha256B64url } from "./handoff-security";
...
/** The Auth0 API identifier (same value as the control plane's AUTH_AUDIENCE). MUST
 *  be requested on /authorize, or Auth0 returns an opaque access token instead
 *  of a JWT the control plane can verify. It's a public, stable identifier — not a
...
export function safeReturn(
  ...
  try {
    ...
    // Preserve the historical marketing-domain callback alias.
...
// custom claims to be namespaced). Keep in sync with apps/control-plane/src/auth.ts CLAIM_NS
 ...
 *  unverified email — the invitation accept flow binds an invite to the
 *  verified email claim as its anti-takeover control. */
export async function createSession(
  ...
  // this. (Backend mirrors this in apps/control-plane/src/auth.ts.)
