...
async function credentialBinding(tx:Tx,scope:EngineScope,input:Admission,actor:CloudRecordedActor,allowStale=false):Promise<Binding>{
    ...
    const row=(await tx.query<Binding>(`SELECT credential.id,credential.owner_user_id,credential.kind,credential.revision,credential.current_version,
        ...
        (material.material_expires_at IS NULL OR material.material_expires_at>now()+interval '1 minute') AS material_ready,
        material.material_expires_at<=now()+interval '10 minutes' AS refresh_due,
        least(now()+interval '45 seconds',delegation.expires_at,material.material_expires_at) AS lease_expires_at
      ...
      WHERE delegation.id=$1 AND delegation.workspace_id=$2 AND delegation.org_id=$3 AND delegation.grantee_user_id=$4
        AND credential.revoked_at IS NULL AND delegation.revoked_at IS NULL AND delegation.expires_at>now()+interval '5 seconds'
...
export async function assertCloudAgentExecutionActor(tx:Tx,scope:EngineScope,executionId:string,actorSessionId:string){
  ...
  const lease=(await tx.query<Lease>(`SELECT * FROM cloud_agent_execution_leases
    WHERE execution_id=$1 AND workspace_id=$2 AND org_id=$3 AND engine_instance_id=$4 AND generation=$5
      AND released_at IS NULL AND expires_at>now()`,[executionId,scope.workspaceId,scope.organizationId,scope.engineInstanceId,scope.generation])).rows[0];
  if(!lease)rejected();
  ...
  const grant=(await tx.query<{id:string}>(`SELECT id FROM cloud_agent_credential_delegations
    WHERE credential_id=$1 AND credential_revision=$2 AND workspace_id=$3 AND org_id=$4 AND grantee_user_id=$5
      ...
      AND revoked_at IS NULL AND expires_at>now()+interval '5 seconds' AND $10=ANY(models)
...
export class DatabaseCloudAgentExecutionService {
  ...
  async authorizeAction(scope:EngineScope,executionId:string,actorSessionId:string){
    return this.transaction(async tx=>{
      ...
      await assertCloudAgentExecutionActor(tx,scope,executionId,actorSessionId);
  ...
  async admit(scope:EngineScope,value:unknown){
    ...
    const result=await this.transaction(async tx=>{
      ...
      if(previous){
        ...
        if(!(await tx.query("SELECT 1 FROM cloud_agent_execution_leases WHERE id=$1 AND expires_at>now()",[previous.id])).rowCount)rejected();
      }
      ...
      if(attempt===0&&binding.kind==="codex-chatgpt"&&binding.refresh_due){
        const renewal=await this.codexRenewal.reserve(tx,binding);if(renewal)return {renewal};
      }
      ...
      if(!previous){
        const count=(await tx.query<{n:number}>(`SELECT count(*)::int AS n FROM cloud_agent_execution_leases
          WHERE workspace_id=$1 AND released_at IS NULL AND expires_at>now()`,[scope.workspaceId])).rows[0]!.n;
  ...
  async validate(scope:EngineScope,leaseId:string,renew=false,credentialVersion?:number,refresh=false){
    ...
    const result=await this.transaction(async tx=>{
      ...
      const lease=(await tx.query<Lease>(`SELECT * FROM cloud_agent_execution_leases WHERE id=$1 AND workspace_id=$2 AND org_id=$3 AND engine_instance_id=$4
        AND generation=$5 AND released_at IS NULL AND expires_at>now()`,[leaseId,scope.workspaceId,scope.organizationId,scope.engineInstanceId,scope.generation])).rows[0];
      if(!lease)rejected();
      ...
      if(attempt===0&&binding.kind==="codex-chatgpt"&&(force||binding.refresh_due)){
        const renewal=await this.codexRenewal.reserve(tx,binding,force);if(renewal)return {renewal};
      }
      ...
      const current=await tx.query(`UPDATE cloud_agent_execution_leases SET expires_at=CASE WHEN $2 THEN $3 ELSE expires_at END
        WHERE id=$1 AND released_at IS NULL AND expires_at>now()`,[leaseId,renew,binding.lease_expires_at]);
