  ...
  const remedy =
    cause.status === 404 && cause.code === "not_found"
      ? " — this control plane serves no /v1/github/* routes at all; the " +
        "control-plane deployment is behind main; redeploy it"
...
function throwGithubAppCommandError(
  ...
  if (error instanceof GithubAppFlowError) {
    ...
    // identically whether the control plane was undeployed (404), deployed without a
 ...
 * that survives (see packages/protocol/src/native-error.ts). */
...
export const ghCredentialClear: CommandHandler = async (args) => {
  ...
  if (method === "github-app") {
    try {
      ...
      // control-plane outage keeps the durable rotating pair for the quiet scheduler
