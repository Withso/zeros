...
//       renderer's JS heap, so an XSS active during sign-in can't capture it.
...
export const authRedeemHandoff: CommandHandler = async (args) => {
  ...
  try {
    ...
    console.warn(
      `[auth] handoff redeem network error: ${(err as Error).message}`,
    );
  ...
  try {
    ...
    console.warn(
      `[auth] handoff persistSession failed: ${(err as Error).message}`,
    );
