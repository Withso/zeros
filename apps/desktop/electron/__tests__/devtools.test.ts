...
//
// apps/desktop/electron/devtools.ts is importable here only because its `electron` import is
...
// must hold for apps/desktop/electron/main.ts, which DOES import electron for real.
...
describe("no build force-closes DevTools", () => {
  const MAIN_SRC = readFileSync("apps/desktop/electron/main.ts", "utf8");
  const DEVTOOLS_SRC = readFileSync("apps/desktop/electron/devtools.ts", "utf8");
