...
// (apps/desktop/electron/preload.ts), so an open console really can read live credentials out
...
// <userData>/app-settings.json (apps/desktop/electron/ipc/commands/app.ts) so MAIN can read
...
// which is why apps/desktop/electron/__tests__/updater-channel-feeds.test.ts has to parse
 ...
 *
 *  Runs through the renderer's PATCHED console (apps/desktop/src/renderer/platform/observability/logging/renderer-log.ts)
 ...
 *  rule apps/desktop/electron/deep-link.ts follows for the URL scheme). */
