 ...
 *  control plane must never delete a credential that still works — but it is not a
...
function mapClientError(error: unknown): GithubAppFlowError {
  ...
  if (error instanceof GithubAppClientError) {
    ...
    // itself may be perfectly healthy. Rotating the control plane's binding key
    ...
    // generic 404 (`code: "not_found"` — `app.notFound` in apps/control-plane/src/index.ts),
    ...
    // terminal: a rolled-back or half-configured control plane must not delete an App
    // credential that still works (see refreshUnderLock).
    //
    // The two are NOT the same thing to an operator, though, and the fix
    // differs: "deploy the control plane" versus "set GITHUB_APP_* and register an
