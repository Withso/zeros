...
import { CanvasKnobHandles, type CanvasKnobAxis } from "./canvas-knob-handles";
...
export function BrowserVariantFrame({
  ...
  const handleCopyHtml = useCallback(() => {
    // Captured HTML can carry scripts or event handlers from the forked page.
  ...
  const startResize = useCallback(
    (e: React.PointerEvent, axis: CanvasKnobAxis, handle: HTMLElement) => {
      ...
      const onMove = (ev: PointerEvent) => {
        ...
        if (axis === "bottom") {
          lastH = Math.max(BROWSER_VARIANT_MIN_HEIGHT, Math.round(startH + dy));
        }
        setDragDims({
  ...
  return (
    ...
    >
      <div
        className="bg-bg1 ring-fg1/5 relative flex flex-col overflow-visible shadow-md ring-1"
        style={{ width, height }}
      >
        <div
          className="border-border1 bg-bg2 text-xxs flex h-[34px] shrink-0 cursor-grab items-center gap-1 border-b px-2 select-none active:cursor-grabbing"
        ...
        >
          {editing ? (
            <Input
              ...
              className="text-xxs h-6 min-w-0 flex-1 px-1.5"
              autoFocus
            />
          ) : (
            <Tooltip label="Drag to move">
              <span
                className="text-fg1 min-w-0 flex-1 cursor-grab truncate font-medium active:cursor-grabbing"
                onDoubleClick={(e) => {
                  e.stopPropagation();
          ...
          <Tooltip label="Copy HTML + CSS">
            <button
              ...
              onPointerDown={(e) => e.stopPropagation()}
              className="text-fg2 hover:bg-bg2-hover hover:text-fg1 inline-flex size-6 items-center justify-center rounded-sm"
              aria-label="Copy HTML and CSS"
            >
              {copied ? (
                <Check className="text-green-primary size-3" />
              ) : (
        ...
        <div className="relative min-h-0 flex-1 overflow-hidden">
          <iframe
            ...
            title={variant.name}
            // No `allow-same-origin`: the static preview never needs the
