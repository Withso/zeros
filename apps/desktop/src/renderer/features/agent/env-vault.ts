...
// ──────────────────────────────────────────────────────────
//
// An environment variable added in Settings → Environment (user scope) or a
// repository page's
...
import { resolveBridgeRepoRootForCwd } from "../../platform/bridge/workspace-id-resolver";
import type { RuntimeClient } from "../../platform/bridge/ws-client";
import { loadProjects, normalizeProjectRoot } from "../../state/projects-store";
 ...
 *  (allowlisted in apps/desktop/electron/keychain-accounts.ts) so it can never collide with
 ...
 *  value. Same literal as MCP_SECRET_SENTINEL (agent-extensions/mcp-server-model.ts);
