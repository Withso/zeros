...
// ──────────────────────────────────────────────────────────
//
// The checkpoint strip is a
// vertical stack of small tick marks overlaid on the left edge of the
// transcript, one tick per user message. The bright tick has the same
// width as the rest, so color is the only active cue; it tracks
...
// whose first prompt sits at the top keeps its FIRST tick lit. A
// fitting two-message chat must not flicker onto the
...
// clicking a tick or a popup row scrolls that
...
//      jump-to-latest pill over a fully visible transcript. When the
//      content below the prompt is a screen or
...
//      own.
//   3. SENDING a prompt does NOT route through this flow: top-framing is
 ...
 *  the top of the viewport wins: when several prompts are on screen,
 *  the TOPMOST takes precedence. Above the first
 *  prompt → first.
 *
 ...
 *  jump-to-latest pill stays hidden. An earlier revision padded this
 *  with 48px of slack to keep
...
interface CheckpointRailProps {
  ...
  /** Imperative handle that lets AgentChat
...
export const CheckpointRail = memo(function CheckpointRail({
  ...
  const scrollToCheckpoint = useCallback(
    (id: string) => {
      ...
      // from the frames flying past. Otherwise clicking the third can
      // light up the fourth once both fit in the viewport.
      ...
      // Long-jump hop: a target many screens
  ...
  // tick is pointless chrome. Render the strip only once a second
  ...
  return (
    <HoverCard openDelay={150} closeDelay={150}>
        ...
        >
          {checkpoints.map((c, i) => (
            <button
              ...
              style={{ height: pitch }}
            >
              {/* Active state is COLOR-ONLY: all
      ...
      {/* Popup placement: ON TOP of the tick
          ...
          tick mark (below): the popup reads as the expanded strip.
          Hover continuity is free — the pointer leaves the
      ...
      <HoverCardContent
        ...
        alignOffset={0}
        collisionPadding={12}
        // gap-1 (4px) between rows + fixed h-8 (32px) rows set the list's
        // rhythm.
      ...
      >
        {checkpoints.map((c, i) => (
          <button
            ...
            // the background only, never the text color. flex-none: h-8
            // must not compress when the
            ...
            className={cn(
              ...
              // uses a darker lightness: the selected row would have
