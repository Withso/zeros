...
describe("relativizePath", () => {
  it("strips the cwd prefix to reveal the in-repo path", () => {
    expect(
      relativizePath(`${CWD}/apps/desktop/src/engine/adapter.ts`, CWD),
    ).toBe("apps/desktop/src/engine/adapter.ts");
  });
...
describe("describePermission", () => {
  it("shows a Read's file (relative) + file icon — the Task 3 fix", () => {
    ...
    const d = describePermission(
      req("other", "Read", {
        file_path: `${CWD}/apps/desktop/src/engine/adapter.ts`,
      }),
      CWD,
    );
    expect(d.detail).toBe("apps/desktop/src/engine/adapter.ts");
