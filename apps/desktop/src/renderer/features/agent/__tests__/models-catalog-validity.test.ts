...
describe("curated model catalog (catalogs/models-v1.json)", () => {
  ...
  it("gates Fable 5.1 / Fable 5 / Sonnet 5 / Opus 5 so none ever silently downgrades", () => {
    ...
    expect(on170.some((w) => w.includes('"claude-fable-5[1m]"'))).toBe(false);
  ...
  it("curates GPT-6 Astra and Cursor Auto / Grok 4.6 without dropping compatibility rows", () => {
    ...
    const byFamily = Object.fromEntries(
      Object.entries(catalog.families).map(([family, models]) => [
        family,
        Object.fromEntries(models.map((model) => [model.value, model])),
      ]),
    );
    ...
    expect(byFamily.cursor.default).toMatchObject({
      effortLevels: [],
      supportsFast: true,
    });
  ...
  it("gives Haiku an empty effort ladder (no toggle) and the right per-model fast flags", () => {
    ...
    expect(claude["claude-fable-5-1[1m]"].supportsFast).toBe(false);
