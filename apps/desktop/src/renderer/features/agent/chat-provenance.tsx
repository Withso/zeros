...
// transcript will be.
//
// Replaces the old "Session ready. Ask the agent anything." line. That string
// had two problems: it said nothing (a ready session is
...
// before the fix in conversation/chat-view.tsx, was on every model/effort/Fast pill
// click.
//
...
//
// The three rows STEP ASIDE the moment this folder has even one prior chat to
// offer. They are not additive with the
// transcript row — see provenanceBlockShape.

import { useMemo } from "react";
import { FolderClosed, GitBranch, Terminal } from "lucide-react";

import { OpenInBadgeMenu } from "@/renderer/shell/conversation/conversation-header";
import { useOpenScriptsSettings } from "@/renderer/shell/workbench/tabs/setup-tab";
import { workspaceLabel } from "@/renderer/shell/workspace-tabs";
import { ZerosSpinner } from "@/renderer/shared/ui/primitives";
 ...
 *
 *  ONE type treatment for the whole block: every
 *  string is `text-sm` (14px in this app; see the text scale in zeros-tokens.css)
      ...
      return (
        <Row icon={<Terminal className="size-3.5" aria-hidden="true" />}>
          <button
            type="button"
            onClick={openScripts}
            // Colour-only hover. An underline
 ...
 *
 *  When the answer is yes, "Add chat transcripts"
...
export function ChatProvenance({
  ...
  return (
    // gap-3 = 12px between rows. At 14px text
