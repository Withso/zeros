...
//
// HTTP header secrets deliberately bypass this environment courier: their
// values live in the engine vault and the MCP gateway adds them while proxying.
//
...
import { deleteSecret, getSecret, setSecret } from "../../platform/secrets";
import { bridgeSettingsRead } from "../../platform/bridge/workspace-bridge";
import type { RuntimeClient } from "../../platform/bridge/ws-client";
import { normalizeProjectRoot } from "../../state/projects-store";
 ...
 *  MUST match the engine) lives in mcp-server-model.ts. */
...
export type McpSecretScope =
  | { kind: "user" }
  | { kind: "repo"; repoRoot: string };

/** Keychain account for an MCP secret env var, under the `mcp::` prefix the
 *  renderer keychain allowlist permits (apps/desktop/electron/keychain-accounts.ts).
...
const SECRET_NAME_RE =
  /(token|secret|password|passwd|pwd|api[_-]?key|access[_-]?key|credential|bearer|auth|[_-]key|^key)/i;
...
export function secretEnvNamesFromDoc(doc: unknown): string[] {
  const servers = (doc as { mcp?: { servers?: unknown } } | null | undefined)
    ?.mcp?.servers;
