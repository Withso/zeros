...
//     Administration tabs flash in and vanish.
...
const { getSettingMigrated, setSetting } =
...
describe("getSettingMigrated", () => {
  ...
  it("adopts the legacy value, rewrites it under the new key, and clears the old", () => {
    ...
    expect(
      getSettingMigrated<string | null>(
        "team:active-id",
        "org:active-id",
        null,
      ),
  ...
  it("carries `false` across — a falsy persisted value is a real answer, not 'unset'", () => {
    ...
    // null (unknown, do not gate yet). Collapsing false→null would reopen access.
    setSetting("org:has-orgs", false);

    expect(
      getSettingMigrated<boolean | null>(
        "team:has-teams",
        "org:has-orgs",
        null,
      ),
