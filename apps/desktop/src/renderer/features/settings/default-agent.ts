...
//   - Conversation pane "+" menu (star toggle on each agent row + autopick
...
/** Hardcoded fallback agent when no preferred default is available.
 *
 *  New chats must not land in a no-agent state. The fallback is Claude, and
 *  the constant lives here so every surface that resolves
 *  "what agent should this new chat use?" — the dispatcher, the
 *  hydration backfill, ChatView's safety net — agrees on one
 ...
 *    2. FALLBACK_AGENT_ID ("claude") — if it's
