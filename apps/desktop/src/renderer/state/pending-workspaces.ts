...
//      Workbench hides both tab rows (Open file/Changes/Review and
...
//      (clearWorkspaceSettling — driven by Workbench after the row exists and
...
interface PendingWorkspacesState {
  ...
  /** Folders (worktree paths) whose workspace just landed and whose workbench
...
/** Before navigation: mark the announced path as settling so Workbench shows
...
/** Workbench declares the confirmed row's cold surface ready (or failed open).
 * Idempotent. */
 ...
 * archived even while Workbench is still assembling its first snapshot. */
