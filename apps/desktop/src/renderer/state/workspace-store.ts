...
// ──────────────────────────────────────────────────────────
//
// This store replaces the React Context + `useReducer` that used to live in
...
import { subscribeToLiveChatDrafts } from "../features/agent/composer-live-drafts";
...
export type Action =
  ...
  // Auto-submit into Conversation pane chat
  ...
  | { type: "ACTIVATE_WORKBENCH_TAB"; id: string; scope?: string }
  | { type: "REORDER_WORKBENCH_TABS"; ids: string[] }
  | {
...
function setWorkbenchScope(
  ...
  scope: string,
  value: WorkbenchScopeState,
): WorkspaceState["workbenchByScope"] {
  return setBoundedRecord(record, scope, value, MAX_PERSISTED_WORKBENCH_SCOPES);
}
...
const initialState: WorkspaceState = {
  ...
  // Workbench tabs, hydrated per worktree (keyed by folder path). See
  // selectWorkbench / workbenchScopeKey below.
  workbenchByScope: loadScopes(),
};

// ──────────────────────────────────────────────────────────
// Active-workspace folder resolution (shared by every Conversation pane surface)
// ──────────────────────────────────────────────────────────
 ...
 *  the old `updatedAt DESC` pick (repository panel's chatsByWorkspace) silently jumped
 ...
 *  repository panel to restore the user's place. */
 ...
 *  chat; Conversation pane renders a dead pane otherwise). Pure; shared by the boot
 *  hydrator, the repository panel/workspace-open handlers, and the tab strip's
...
// ──────────────────────────────────────────────────────────
// Workbench tabs — scoped per worktree (folder path)
// ──────────────────────────────────────────────────────────
...
// (no remount, so workbench's terminal panel and resize state survive).

/** The key under which the ACTIVE worktree's tab slice lives — its folder path,
 *  or a sentinel for the ambient/no-folder scope. */
export function workbenchScopeKey(s: WorkspaceState): string {
  return workbenchScopeForFolder(selectActiveFolder(s));
}

/** Normalize a workspace folder into the same key used by workbenchScopeKey.
 *  Async file operations carry this key explicitly to avoid cross-worktree
 *  races after the active workspace changes. */
export function workbenchScopeForFolder(folder: string | null): string {
  if (!folder) return "__ambient__";
  return folder.replace(/\/+$/, "") || "/";
}

/** The ACTIVE worktree's workbench tab slice, or its cached default home tabs,
...
export function selectWorkbench(s: WorkspaceState): WorkbenchScopeState {
  const scope = workbenchScopeKey(s);
  return s.workbenchByScope[scope] ?? defaultScopeFor(scope);
}
...
function removeWorkbenchTabs(
  cur: WorkbenchScopeState,
  shouldRemove: (tab: WorkbenchTab) => boolean,
): WorkbenchScopeState {
  ...
  switch (action.type) {
    ...
    case "MOVE_WORKSPACE_UI_STATE": {
      ...
      return {
        ...
        workbenchByScope: moveRecordKeysMatching(
          state.workbenchByScope,
          belongsToMovedWorkspace,
          moveFolder,
        ),
    ...
    case "DELETE_CHAT": {
      ...
      // auto-spawns a fresh chat so Conversation pane never shows a dead pane).
    ...
    case "ARCHIVE_CHAT": {
      ...
      // closed chat's folder so the chrome (Repository panel highlight, topbar,
      // tabs strip) doesn't degrade to "No workspace selected". Callers
      // that pick a replacement chat (conversation/pane-layout handleCloseTab)
      ...
      // nulling it — a null selection renders a dead Conversation pane (the
      // EmptyComposer landing was deleted 2026-06-18). Callers with a better
      // pick (conversation/pane-layout' pane-neighbor strategy) dispatch their own
    ...
    // ── Workbench tabs — scoped per worktree (see selectWorkbench) ──
    ...
    case "ADD_WORKBENCH_TAB": {
      ...
      if (
        ...
        if (existing) {
          ...
          return {
            ...state,
            workbenchByScope: setWorkbenchScope(state.workbenchByScope, scope, {
              ...cur,
      ...
      // slice (defaultTabs/normalizeWorkbenchTabs), so a stray caller flag can't
      ...
      const next: WorkbenchScopeState = {
        ...cur,
        ...
        tabs: orderWorkbenchTabs([...cur.tabs, tab]),
        activeId,
        recentBrowsers:
          tab.type === "browser"
    ...
    case "REMOVE_WORKBENCH_TAB": {
      ...
      const next = removeWorkbenchTabs(cur, (t) => t.id === action.id);
      return {
        ...state,
        workbenchByScope: setWorkbenchScope(
          state.workbenchByScope,
    ...
    case "CLOSE_WORKBENCH_FILE_IF_MATCHES": {
      const cur =
        state.workbenchByScope[action.scope] ?? defaultScopeFor(action.scope);
      const target = cur.tabs.find((tab) => tab.id === action.id);
    ...
    case "REORDER_WORKBENCH_TABS": {
      ...
      // Pinned workbench homes keep their canonical slots no matter what order the
      // caller supplied; File tabs remain closable.
      return {
        ...state,
        workbenchByScope: setWorkbenchScope(state.workbenchByScope, scope, {
          ...cur,
          tabs: orderWorkbenchTabs(reordered),
          activeId: cur.activeId,
        }),
      };
    }
    case "UPDATE_WORKBENCH_TAB": {
      const scope = action.scope ?? workbenchScopeKey(state);
      const cur = state.workbenchByScope[scope] ?? defaultScopeFor(scope);
      const target = cur.tabs.find((t) => t.id === action.id);
      if (!target) return state;
      ...
      let updatedTarget: WorkbenchTab = {
        ...target,
        ...action.updates,
    ...
    case "RECONCILE_WORKBENCH_FILE_DISCARD": {
      const scope = workbenchScopeForFolder(action.scope);
      const cur = state.workbenchByScope[scope] ?? defaultScopeFor(scope);
      if (action.outcome === "removed") {
        ...
        const next = removeWorkbenchTabs(
          cur,
      ...
      let changed = false;
      const tabs: WorkbenchTab[] = cur.tabs.map((t) => {
        if (
...
useWorkspaceStore.subscribe((s, prev) => {
  ...
  // Workbench tabs persist per worktree. Centralized here (not in <Workbench>'s
  // effect) so a tab mutation is saved even when workbench is collapsed.
  if (s.workbenchByScope !== prev.workbenchByScope) {
    saveScopes(s.workbenchByScope);
  }
});
...
// call site, or return a primitive — see conversation/terminal-deck / the
...
/** One-shot auto-submit hand-off into Conversation pane's chat, or null. */
...
/** The ACTIVE worktree's Workbench tab list. Re-renders only when this
 *  worktree's tabs change or the active worktree switches — selectWorkbench
 *  returns a stable slice reference per scope, so Object.is suppresses the
 *  rest. */
export function useWorkbenchTabs(): WorkbenchTab[] {
  return useWorkspaceStore((s) => selectWorkbench(s).tabs);
}

/** The active Workbench tab id for the active worktree, or null. */
export function useActiveWorkbenchTabId(): string | null {
  return useWorkspaceStore((s) => selectWorkbench(s).activeId);
}

/** Recently visited Browser pages for the active workspace, newest first. */
export function useRecentWorkbenchBrowsers(): WorkbenchScopeState["recentBrowsers"] {
  return useWorkspaceStore((s) => selectWorkbench(s).recentBrowsers);
}
