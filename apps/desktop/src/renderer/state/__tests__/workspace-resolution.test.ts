...
describe("repoSlugFromWorktreePath", () => {
  ...
  it("extracts the slug from the current ~/zeros/workspaces root", () => {
...
describe("resolveWorkspacePresentationKind", () => {
  const designPath = "/Users/dev/zeros/design workspaces/widgets/landing-page";
  ...
  it("keeps the confirmed surface mounted until a mode switch is committed", () => {
    expect(
      resolveWorkspacePresentationKind({
        confirmedKind: "code",
  ...
  it("uses pending and managed-path identity only before a row confirms", () => {
    ...
    expect(resolveWorkspacePresentationKind({ folder: designPath })).toBe(
      "design",
    );
    expect(resolveWorkspacePresentationKind({ folder: "/tmp/plain" })).toBe(
      "code",
    );
  });
});
