...
function assertCleanProps(props: Record<string, unknown>): void {
  for (const [k, v] of Object.entries(props)) {
    expect(["string", "number", "boolean", "undefined"], `prop ${k} type`).toContain(
      typeof v,
    );
    expect(piiShaped(v), `prop ${k}=${JSON.stringify(v)} looks like PII`).toBe(false);
  }
}
...
describe("agent-events PII contract — emits scalar metadata only", () => {
  it("every builder's emitted props are scalar metadata (no paths/emails/free text)", () => {
    ...
    trackAgentSessionEnded({ agentId: "claude", chatId: "chat_1", outcome: "completed" });
  ...
  it("trackGitOp scrubs a raw error to a safe enum — a path/email in it never reaches analytics", () => {
    ...
    trackGitOp({
      op: "commit",
      outcome: "error",
      error: new Error("failed at /Users/alice/secret/repo by alice@example.com"),
    });
    ...
    capture.mockClear();
    trackGitOp({ op: "push", outcome: "error", error: { code: "REMOTE_REJECTED" } });
  ...
  it("trackAgentFailed builds the exception from enums only; non-bug kinds don't raise", () => {
    trackAgentFailed({ agentId: "cursor", failure: { kind: "protocol-error", stage: "prompt" } });
    ...
    captureException.mockClear();
    trackAgentFailed({ agentId: "cursor", failure: { kind: "auth-required", stage: "newSession" } });
  ...
  it("trackAgentFailed hashes the technical message — never sends it raw", () => {
    ...
    const eventProps = capture.mock.calls.at(-1)?.[1] as Record<string, unknown>;
    const excProps = captureException.mock.calls.at(-1)?.[1] as Record<string, unknown>;
    ...
    trackAgentFailed({
      agentId: "cursor",
      failure: { kind: "protocol-error", stage: "newSession", message: "TLS handshake rejected" },
    });
    ...
    capture.mockClear();
    trackAgentFailed({ agentId: "cursor", failure: { kind: "timeout", stage: "prompt" } });
    expect(
