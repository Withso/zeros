...
// workspace path so a bare/partial mention still opens the right file in workbench.
describe("pickFileMatch", () => {
  const FILES = [
    "package.json",
    "src/styles/variables.css",
    "src/features/agent/AskAIChat.tsx",
    "src/features/MyAskAIChat.tsx",
    "src/a/util.ts",
    "src/b/c/util.ts",
  ];
  ...
  it("anchors the basename at a path boundary (no substring false-positives)", () => {
    ...
    expect(pickFileMatch(["src/features/MyAskAIChat.tsx"], "AskAIChat.tsx")).toBe(
      null,
    );
  });
