...
describe("shared PR status row placement", () => {
  it("mounts one row above the retained Changes and Review bodies", () => {
    const column = source("apps/desktop/src/renderer/shell/workbench/workbench-pane.tsx");
    const changes = source("apps/desktop/src/renderer/shell/workbench/tabs/changes-surface.tsx");
    const review = source("apps/desktop/src/renderer/shell/workbench/tabs/review-surface.tsx");
...
describe("Create PR routing", () => {
  it("sends the primary action to the agent and labels the engine path explicitly", () => {
    const button = source("apps/desktop/src/renderer/shell/pr/create-pr-button.tsx");
...
describe("PR prompt single-flight wiring", () => {
  it("claims before sending and releases only when the accepted turn settles", () => {
    const island = source("apps/desktop/src/renderer/shell/pr/pr-status-island.tsx");
