...
// Workbench → PrStatusRow). It shows a single
...
//
// Two rules keep the status row from presenting transient mixed state:
...
//     so an app relaunch or dev reload resumes from it too — the
//     refetch still runs, in the background, and repaints only on an actual
//     status change. "Loading..." (plain text, no spinner)
...
import { CommitPushIcon } from "../../features/agent/auto-action";
...
import {
  ...
  statusForGeneration,
} from "../workbench/tabs/changes-tab";
...
// Tone → row surface + bottom hairline. Every open-PR state shares the brown
// identity; green, violet, and red mark terminal states.
...
function actionButtonClass(action: PrIslandAction, tone: PrIslandTone): string {
  if (action.variant === "primary") {
    ...
    // the salmon surface — flip to a white bordered button.
    return cn(
 ...
 *  unmount/remount cycle (inactive workbench tabs unmount) so a tab switch
...
export function PrStatusIsland({
  ...
  // Focus workbench's pinned Review tab — shared by the View-PR chip and the
  ...
  /** Direct engine actions: push, pull, ready, merge, and continue. They emit no
   *  agent message; the button shows a spinner and applies an optimistic state. */
  ...
  return (
    <div
      data-pr-island=""
      className={cn(
        // One-pixel hairlines above and below the tinted flush row frame it
