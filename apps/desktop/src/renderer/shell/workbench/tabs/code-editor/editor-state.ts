// ──────────────────────────────────────────────────────────
// Workbench editor dirty registry — which File tabs hold unsaved edits?
// ──────────────────────────────────────────────────────────
//
...
// draft. This registry is the small external-store seam that tells Workbench
...
export function setWorkbenchEditorDirty(editorId: string, value: boolean): void {
...
export function isWorkbenchEditorDirty(editorId?: string): boolean {
...
/** Reactive dirty-id snapshot for Workbench and file-open navigation. */
export function useWorkbenchDirtyEditorIds(): ReadonlySet<string> {
  return useSyncExternalStore(subscribe, snapshot, () => EMPTY_DIRTY_EDITORS);
}

/** Test-only reset for the module singleton. */
export function resetWorkbenchEditorDirtyForTests(): void {
