...
describe("mergeIgnoredPaths", () => {
  ...
  it("dedupes a nested root that also appears as a loaded child", () => {
    // `git ls-files --directory` reports packages/protocol/node_modules/ as its own
    // root AND it turns up when the user expands packages/protocol — feeding both
    // to the tree would render the row twice.
    const merged = mergeIgnoredPaths(
      ["node_modules/", "packages/protocol/node_modules/"],
      map([["packages/protocol", ["packages/protocol/node_modules/"]]]),
    );
    expect(
      merged.filter((p) => p === "packages/protocol/node_modules/"),
    ).toHaveLength(1);
  });
