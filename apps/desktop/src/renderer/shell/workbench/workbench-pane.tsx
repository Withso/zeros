...
} from "./tabs/code-editor/editor-state";
import { SETUP_SUBTAB } from "../terminal/use-setup-control";
import { WorkbenchTabStrip } from "./tab-strip";
import { WorkbenchToggleButton } from "./toggle-button";
...
import { shouldInitializeFreshWorkspace } from "./fresh-workspace";
...
// instead of flowing entirely into workbench. `flex-basis: 0` lets the
// grow factors decide the split; `min-w-[200px]` floors it. The ×100
// scale keeps the grow sum ≥ 1 so workbench absorbs ALL the remainder
// when conversation pane freezes at its 2400px cap on ultrawide windows (bare
...
// seam against conversation pane. No top inset (2026-07-12): the h-10 header is the
// column's full 0..40px title strip, like repository panel and conversation pane.
const WORKBENCH_PANE_CLS =
...
const MAX_RETAINED_WORKBENCH_VIEWS = 6;
 ...
 * main Workbench component means a hidden workspace's title/URL update does not
...
function RetainedBrowserDeck({
  ...
  const activeScope = useWorkspaceStore(workbenchScopeKey);
  const targets = useMemo(() => {
    const next = new Map<string, BrowserViewTarget>();
    for (const [scope, slice] of Object.entries(workbenchByScope)) {
...
const RetainedBrowserView = React.memo(function RetainedBrowserView({
  ...
  return (
    <div
      ...
      aria-hidden={!active}
    >
      <WorkbenchTabContent tab={tab} active={active} scope={scope} />
    </div>
  );
});
...
// Workbench can unmount when the whole column is collapsed. Keep the creation
...
const workbenchSessionStartedAt =
  typeof performance !== "undefined" && Number.isFinite(performance.timeOrigin)
    ? performance.timeOrigin
    : Date.now();

interface WorkbenchPaneProps {
  /** Provided by app-shell — toggles workbench visibility. Rendered in the
   *  header. */
  onToggleWorkbench: () => void;
  /** False while the persistent workspace route is hidden behind Home. */
  surfaceActive?: boolean;
  /** Keep the view deck mounted but out of layout while Workbench is closed. */
  collapsed?: boolean;
}
...
export function WorkbenchPane({
  ...
  const activeId = tabs.some((t) => t.id === storedActiveId)
    ...
    : (tabs.find((t) => t.type === "files")?.id ??
      ...
      tabs[0]?.id ??
      null);
  const activeWorkbenchTab = tabs.find((tab) => tab.id === activeId) ?? null;
  ...
  const workbenchIdsToRetain = useRetainedViewKeys(
    activeId,
    MAX_RETAINED_WORKBENCH_VIEWS,
  ...
  useLayoutEffect(() => {
    ...
    if (wasCollapsed && !collapsed) restoreScrollWithin(workbenchRef.current);
  }, [collapsed]);
  ...
  // mid-work in workbench: any interaction (click / scroll / typing) in the
  ...
  const { folderKey, chatCwd } = useWorkbenchFolder();
  useInstantViewSwitch(
    surfaceActive ? `${folderKey}:${activeId ?? "empty"}` : "workbench:hidden",
    workbenchRef,
  );
  ...
  useEffect(() => {
    ...
    // workbench); the timeout guarantees the reveal even if that load fails.
  ...
  const lastWorkbenchInteractionRef = useRef(0);
  const markWorkbenchInteraction = () => {
    lastWorkbenchInteractionRef.current = Date.now();
  };
  ...
  useEffect(() => {
    ...
    if (
      !shouldInitializeFreshWorkspace({
        createdAt,
        sessionStartedAt: workbenchSessionStartedAt,
      })
    ) {
  ...
  const retainedWorkbenchSet = new Set(workbenchIdsToRetain);
  ...
  return (
    ...
    // typing anywhere in workbench = "user is busy here"); events inside the
    ...
    <DiffWorkerPoolProvider>
      <div
        ...
        data-zeros-root=""
        onPointerDownCapture={markWorkbenchInteraction}
        onWheelCapture={markWorkbenchInteraction}
        onKeyDownCapture={markWorkbenchInteraction}
      >
        {settling ? (
