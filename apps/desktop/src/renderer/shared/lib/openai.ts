...
import type { AiSettings } from "../../state/store";
...
// keychain via apps/desktop/src/renderer/platform/secrets.ts and is hydrated asynchronously
...
export function loadAiSettings(): AiSettings {
  try {
    ...
    if (saved) {
      ...
      // from legacy builds; it is about to be migrated to the
...
async function keySlotFor(provider: AiSettings["provider"]) {
  const { SECRET_ACCOUNTS } = await import("../../platform/secrets");
  return provider === "claude"
    ? SECRET_ACCOUNTS.ANTHROPIC_API_KEY
    : SECRET_ACCOUNTS.OPENAI_API_KEY;
}

export async function hydrateAiApiKey(
  settings: AiSettings,
): Promise<AiSettings> {
  const { getSecret, setSecret, SECRET_ACCOUNTS } =
