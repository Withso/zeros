...
export class CloudReplicaSyncBroker {
  ...
  private readonly inFlight = new Map<string, Promise<CloudReplicaLocalState>>();
  ...
  sync(replicaId: string): Promise<CloudReplicaLocalState> {
    ...
    const work = this.syncExclusive(replicaId).finally(() => {
      if (this.inFlight.get(replicaId) === work) this.inFlight.delete(replicaId);
    });
  ...
  private updateRemote(local: CloudReplicaLocalState, remote: CloudReplicaRemoteState) {
  ...
  private async activeGrant(local: CloudReplicaLocalState): Promise<string> {
    ...
    const projectionHash = this.state.projection(local.replicaId).manifestSha256();
  ...
  private async applyOrReport(
    ...
    try {
      ...
      const included = input.mutations.filter((mutation) =>
        cloudReplicaPathIncluded(policy, mutation.path),
      );
      const applyToRevision =
        included.at(-1)?.revision ?? input.fromRevision;
      const result =
        included.length === 0
  ...
  private async bootstrap(
    ...
    for (let pageIndex = 0; pageIndex < MAX_BOOTSTRAP_PAGES; pageIndex += 1) {
      ...
      if (
        (descriptor && JSON.stringify(descriptor) !== JSON.stringify(currentDescriptor)) ||
        page.checkpointId !== local.checkpointId ||
        page.manifestRevision !== local.manifestRevision ||
        (cursor !== null && page.entries.length > 0 && page.entries[0]!.path === cursor)
      ) {
        throw new CloudReplicaBrokerError(
          "remote_protocol_error",
      ...
      const mutations = page.entries.map((entry) => ({
        ...entry,
        revision: page.manifestRevision,
        sequence: (sequence += 1),
      }));
  ...
  private async refreshSnapshot(
    ...
    assertRemoteIdentity(local, response.replica);
    if (!response.replica.checkpointId || response.replica.manifestRevision === null) {
      throw new CloudReplicaBrokerError(
        "remote_protocol_error",
  ...
  private async syncExclusive(replicaId: string): Promise<CloudReplicaLocalState> {
    ...
    for (let pageIndex = 0; pageIndex < MAX_EVENT_PAGES_PER_RUN; pageIndex += 1) {
      local = this.local(replicaId);
      ...
      if (
        page.fromRevision !== local.eventCursor ||
        page.toRevision < page.fromRevision ||
        page.toRevision > page.currentRevision ||
        (page.events.length > 0 &&
          page.events[page.events.length - 1]!.revision !== page.toRevision) ||
