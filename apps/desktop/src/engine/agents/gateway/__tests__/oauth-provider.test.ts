...
describe("ZerosOAuthProvider", () => {
  const make = (over: Partial<Parameters<typeof makeDeps>[0]> = {}) => {
    ...
    const provider = new ZerosOAuthProvider(
      makeDeps({ vault, openBrowser, ...over }),
    );
  ...
  it("clientInformation: vault registration > static client id > undefined", () => {
    ...
    expect(stat.provider.clientInformation()).toEqual({
      client_id: "preconfigured",
    });

    const reg = make();
    reg.provider.saveClientInformation({
      client_id: "dcr-id",
      redirect_uris: ["http://127.0.0.1:24302/callback"],
    });
  ...
  it("saveTokens persists the whole object so a rotated refresh_token wins", () => {
    const { provider, vault } = make();
  ...
  it("saveDiscoveryState enforces issuer binding against mix-up", () => {
    const { provider } = make();
