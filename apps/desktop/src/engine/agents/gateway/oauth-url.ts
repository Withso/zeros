...
// caller before networking begins.
// ──────────────────────────────────────────────────────────
...
export function issuerMismatchReason(
  issuer: string,
  ...
  const norm = (u: URL) =>
    `${u.protocol}//${u.host}${u.pathname.replace(/\/+$/, "")}`.toLowerCase();
  if (norm(a) !== norm(b)) {
...
export function parseAuthorizationCode(input: string): {
  code: string;
  state?: string;
} {
...
export function unsafeAuthUrlReason(
  raw: string,
  opts: AuthUrlGuardOptions = {},
): string | null {
  let u: URL;
  try {
    u = new URL(raw);
...
export function assertSafeAuthUrl(
  raw: string,
  opts: AuthUrlGuardOptions = {},
): URL {
