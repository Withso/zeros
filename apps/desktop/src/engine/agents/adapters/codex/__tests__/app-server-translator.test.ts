...
describe("CodexAppServerTranslator", () => {
  ...
  describe("context compaction → two-state tool row", () => {
    // "Compacting.." while the item runs → "Context
    ...
    // User-initiated compaction (Compact now / /compact) arms
    // expectManualCompaction before the RPC and
  ...
  describe("token usage → usage_update (context gauge)", () => {
    // `used` must be the current window fill (last call's full
