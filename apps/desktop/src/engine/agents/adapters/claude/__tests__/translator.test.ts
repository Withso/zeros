...
// any of them as a top-level user message causes duplicated user bubbles or
// misattributes a subagent message to the user. Tool results must still surface;
// assistant text is unaffected.
...
describe("ClaudeStreamTranslator local workflow progress", () => {
  ...
  it("preserves an out-of-order pause edge and never reopens after a terminal edge", () => {
    ...
    const pausedFrame = paused.updates
      .filter(
        (note) => note.update.sessionUpdate === "workflow_progress_update",
      )
    ...
    terminal.t.feed({
      ...
      subtype: "task_progress",
      task_id: "workflow-done",
      workflow_progress: [{ type: "workflow_phase", index: 0, title: "Find" }],
    });
    expect(
...
describe("ClaudeStreamTranslator compact_boundary", () => {
...
describe("ClaudeStreamTranslator result usage_update (zero-dip guard, 2026-07-12)", () => {
  ...
  it("reports the 1M window for every 1M model the picker offers", () => {
    ...
    // unavailable). The rule now lives in @zeros/protocol, shared with the
...
describe("ClaudeStreamTranslator per-model usage", () => {
