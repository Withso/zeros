...
if (descriptor.env.HOST_PARITY_CANARY) {
  ...
  const result = {
    codeWrite: spec.actor !== "design-agent",
    designWrite: spec.designDirectories.map(() => false),
    designAliasWrite: spec.designDirectories.length > 0 ? false : true,
...
process.stdout.write(JSON.stringify({
  ...
  (process.platform === "linux" ? it : it.skip)(
    "enforces API-only Design-agent writes in the real Linux runtime",
    async () => {
      const design = path.join(workspace, "Zeros Design");
      const designFile = path.join(design, "canvas.json");
      const outside = path.join(root, "outside-cache");
      const managedWorkspaces = path.join(root, "managed-workspaces");
      const engineSecret = path.join(
        process.env.ZEROS_DATA_DIR!,
        "engine-secret.txt",
      );
      await Promise.all(
        [design, managedWorkspaces, process.env.ZEROS_DATA_DIR!].map(
          (directory) => mkdir(directory, { recursive: true }),
        ),
      );
      await writeFile(designFile, "before\n");
      await writeFile(engineSecret, "engine-only\n");
      await runFile("git", ["init", "-b", "main"], { cwd: workspace });
...
const fs = require("node:fs");
const { spawnSync } = require("node:child_process");
const [codeFile, designFile, outsideFile, futureWorkspaceFile, engineSecret] = process.argv.slice(1);
...
let designDenied = false;
try { fs.writeFileSync(designFile, "after\n"); } catch { designDenied = true; }
let outsideDenied = false;
try { fs.writeFileSync(outsideFile, "cache\n"); } catch { outsideDenied = true; }
let scratchWritable = false;
try { fs.writeFileSync(process.env.TMPDIR + "/artifact.txt", "artifact\n"); scratchWritable = true; } catch {}
let engineReadDenied = false;
try { fs.readFileSync(engineSecret, "utf8"); } catch { engineReadDenied = true; }
...
process.stdout.write(JSON.stringify({
  ...
  gitStderr: git.stderr,
        ...
        expect(JSON.parse(stdout)).toEqual({
          ...
          engineReadDenied: true,
          design: "before\n",
          gitStatus: 128,
          gitStderr: expect.any(String),
        });
...
process.stdout.write(JSON.stringify({
  ...
  it("reports container workflows unavailable to a Design actor", async () => {
    const boundary = new ZsrExecutionBoundary({
      projectRoot: root,
      supervisorScript: supervisor,
      supervisorRuntime: process.execPath,
