// Workspace Git regression coverage: byte-exact paths, lossless archive and
// restore, detached-HEAD commits, merge-base diffs, disk seeding, and adopted
// branch uniqueness.
...
import { getWorkspaceById, seedFromDisk, worktreesRoot } from "../state";
...
async function initRepo(repoRoot: string): Promise<void> {
  ...
  await git(
    ...
    "add",
    "origin",
    "https://example.com/t/regressions.git",
  );
...
describe("workspace Git regressions", () => {
  let workdir: string;
  let repoRoot: string;
  ...
  beforeEach(async () => {
    workdir = await mkdtemp(path.join(tmpdir(), "zeros-git-regressions-"));
  ...
  describe("porcelain -z byte-exact paths", () => {
  ...
  describe("archive captures untracked files", () => {
    it("round-trips both tracked edits and untracked files", async () => {
      ...
      // Archive captures a durable snapshot (refs/zeros/archive/<id>), not a
      // stash; untracked work must still survive.
      ...
      // The untracked file must survive the round trip.
  ...
  describe("commit on detached HEAD", () => {
  ...
  describe("branch uniqueness on adopt", () => {
