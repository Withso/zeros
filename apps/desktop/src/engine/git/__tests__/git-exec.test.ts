...
describe("workspace git/gh credential shims", () => {
  ...
  it("falls back when a stale broker socket names a live reused PID", async () => {
    setGitCredentialSourceForTesting({
      supports({ protocol, host }) {
        return protocol === "https" && host === "github.com";
      },
    ...
    const fallbackHelper = path.join(dir, "fallback-helper-live-pid");
    await writeFile(
      fallbackHelper,
      "#!/bin/sh\nprintf 'username=ambient\\npassword=surviving-helper\\n'\n",
    );
    ...
    const env = {
      ...process.env,
      ...prepared!.env,
      ZEROS_GIT_AUTH_SOCKET: path.join(dir, "stale-reused-pid.sock"),
    ...
    const result = await runFile(gitShim, ["credential", "fill"], {
      env,
      input: "protocol=https\nhost=github.com\n\n",
    });
