...
// apps/desktop/src/engine/git/__tests__/ because vitest.config.ts is scoped there —
...
describe("gateway resolveAgentCwd", () => {
  ...
  beforeEach(async () => {
    ...
    await execFileAsync(
      "git",
      ["remote", "add", "origin", "https://example.com/g/cwd.git"],
      { cwd: repoRoot },
    );
  ...
  it("throws when the supplied cwd no longer exists on disk", () => {
    // A chat bound to a since-deleted worktree must fail loudly with
