...
// zeros.db (db/database.ts), reached by every surface through the engine
...
/** Test seam — point the DB + worktrees at a temporary directory without
 *  spawning a whole Electron app. The workspace tables live in unified zeros.db,
...
/** The legacy hidden worktrees root (~/.zeros/worktrees). Used by the
 ...
 *  the unified engine zeros.db (created by migration 7). This
...
// restricted workspace is hidden from every remote relay client — withheld
...
// ──────────────────────────────────────────────────────────
// detach_state — singleton row (id = 1), used by detach mode.
// ──────────────────────────────────────────────────────────
...
function seedFromAppData(): { inserted: number; skipped: number } {
  ...
  for (const keyDir of keyDirs) {
    ...
    // Never reconstruct without a real repoRoot, branch, and path.
...
function seedFromRoot(root: string): { inserted: number; skipped: number } {
  ...
  for (const repoSlug of repoDirs) {
    ...
    for (const wsDir of workspaceDirs) {
      ...
      if (!seed.id) continue;
      // Never reconstruct a workspace without a real repoRoot and branch.
