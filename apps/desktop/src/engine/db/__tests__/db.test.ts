...
describe("Zeros DB (unified engine store)", () => {
  ...
  it("creates the full v1 schema and records the migration", () => {
    ...
    expect(
      (
        db.prepare("PRAGMA table_info(chats)").all() as { name: string }[]
      ).some((column) => column.name === "native_session_id"),
    ).toBe(true);
    expect(
      (
        db.prepare("PRAGMA table_info(chats)").all() as { name: string }[]
      ).some((column) => column.name === "native_git_info"),
    ).toBe(true);
  });

  it("round-trips the provider-native session identity separately", () => {
    setZerosDbPathForTesting(tmpDbFile());
    upsertChat(
      makeChat("codex-chat", {
        agentId: "codex",
    ...
    expect(listChats()[0]).toMatchObject({
      sessionId: "zeros-runtime-1",
      nativeSessionId: "codex-thread-1",
    });
  });

  it("round-trips provider-native Git metadata without flattening nullable fields", () => {
    setZerosDbPathForTesting(tmpDbFile());
    upsertChat(
      makeChat("codex-git", {
        agentId: "codex",
    ...
    expect(listChats()[0]?.nativeGitInfo).toEqual({
      sha: "abc123",
      branch: "santhosh",
      originUrl: null,
    });
  ...
  // The idealized `messages`/`messages_fts` test was removed because migration
  // 7 drops those unused tables. Real transcript FTS over chat_messages_fts is
  // covered by the "FTS: cross-chat search" test below.
