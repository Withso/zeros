// ──────────────────────────────────────────────────────────
// Chat transcripts — chat_messages in the unified Zeros DB
// ──────────────────────────────────────────────────────────
//
// The engine persists agent messages here AS IT STREAMS them (the persist hook
// in apps/desktop/src/engine/zeros-engine.ts folds chunks with the shared applyUpdate coalescer and
// upserts the changed messages). It's the source — works even when no client is
// attached (for example, after a relay disconnects). Clients read them over the
// bridge (`messages.window`).
//
 ...
 * sync revisions. */
function annotateLegacySteers(chatId: string, rows: MsgDbRow[]): MsgDbRow[] {
...
/** The searchable text for a message in FTS: a text message's `text`,
...
export function upsertChatMessagesBulk(
  chatId: string,
 ...
 *  reader can conclude their message was lost.
 *
...
function snapToTurnStart(chatId: string, rows: MsgDbRow[]): MsgDbRow[] {
  ...
  const db = openZerosDb();
  const start = (db
    .prepare(TURN_START_ORD_SQL)
    .get(chatId, floor, oldest.ord) ??
    db.prepare(TURN_START_ORD_ANY_USER_SQL).get(chatId, floor, oldest.ord)) as
    | { ord: number }
    | undefined;
...
export function truncateChatMessagesFrom(
  chatId: string,
  fromMsgId: string,
): number {
...
/** Full-text search over transcripts (FTS5). Each whitespace token is
 *  quoted (so `relay-core` etc. are literal, not FTS operators) and AND-ed;
 *  results are relevance-ranked.
 *
 *  `scope` restricts the search to a single chat (`chatId`) or workspace
 ...
 *  pull can resume from the last delivered row. The message-table parallel
 *  to listChatsSince. */
 ...
 *  rather than the global head, or rows past the cap would be skipped forever. */
...
export function backfillChatMessageRevs(): number {
  ...
  const rows = db
    .prepare(
      "SELECT chat_id, msg_id FROM chat_messages WHERE rev = 0 ORDER BY chat_id, ord",
    )
    .all() as { chat_id: string; msg_id: string }[];
  if (rows.length === 0) return 0;
  const upd = db.prepare(
    "UPDATE chat_messages SET rev = ? WHERE chat_id = ? AND msg_id = ?",
  );
  const tx = db.transaction(() => {
