 ...
 *  whole point is the narrow allowlist in setup-hooks.ts, which exists because
...
export function buildPtyEnv(opts?: {
  ...
  // E.1 scoping in apps/desktop/electron/ipc/commands/pty.ts.)
  if (opts?.cwd) {
    env.PWD = opts.cwd;
 ...
 *  The allowlist exists because `scripts.setup` is repository-resident and may arrive
