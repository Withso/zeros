// ──────────────────────────────────────────────────────────
// API surface (docs/teams.md Part E). Every team-scoped route:
...
function parse<T>(schema: z.ZodType<T, z.ZodTypeDef, unknown>, value: unknown): T {
  ...
  if (!r.success) {
    throw new HttpError(422, "invalid_input", r.error.issues[0]?.message ?? "Invalid input");
  }
  return r.data;
}
...
export function createRoutes(pool: pg.Pool, email?: EmailConfig): Hono {
  ...
  // (verified against live policies, 2026-07-22 review C1). No team-scoped
  // authz applies anyway — any authenticated user may create a team; the
  // input is Zod-validated and created_by/audit are stamped server-side.
  app.post("/v1/teams", async (c) => {
    ...
    const team = await withSystemTx(pool, async (tx) => {
      ...
      if (!created?.rows[0]) {
        throw new HttpError(409, "slug_conflict", "Couldn't allocate a unique team URL — try again");
      }
  ...
  app.patch("/v1/teams/:team", async (c) => {
    ...
    // name to ride along.
    const name = body.name === undefined ? undefined : parse(NameSchema, body.name);
    const logo =
      body.logo === undefined
        ? undefined
        : body.logo === null
    ...
    const team = await withUserTx(pool, user.id, async (tx) => {
      ...
      if (!updated.rows[0]) throw new HttpError(404, "not_found", "Team not found");
      if (name !== undefined) await audit(tx, teamId, user.id, "team.renamed", { name });
      if (logo !== undefined) {
        await audit(tx, teamId, user.id, "team.logo_updated", { cleared: logo === null });
      }
  ...
  app.delete("/v1/teams/:team", async (c) => {
    ...
    await withUserTx(pool, user.id, async (tx) => {
      ...
      if (!gone.rowCount) throw new HttpError(404, "not_found", "Team not found");
  ...
  app.patch("/v1/teams/:team/members/:user", async (c) => {
    ...
    await withUserTx(pool, actor.id, async (tx) => {
      ...
      if ((current === "owner" || role === "owner") && actorRole !== "owner") {
        throw new HttpError(403, "forbidden", "Only owners can change owner roles");
      }
      if (current === "owner" && role !== "owner") await assertNotLastOwner(tx, teamId);
      await tx.query(
        `UPDATE team_members SET role = $3 WHERE team_id = $1 AND user_id = $2`,
        [teamId, targetId, role],
      );
      await audit(tx, teamId, actor.id, "member.role_changed", { user: targetId, role });
    });
    return c.json({ ok: true });
  });
  ...
  app.delete("/v1/teams/:team/members/:user", async (c) => {
    ...
    await withUserTx(pool, actor.id, async (tx) => {
      ...
      if (current === "owner" && !selfLeave && actorRole !== "owner") {
        throw new HttpError(403, "forbidden", "Only owners can remove an owner");
      }
      ...
      // leave back (verified, 2026-07-22 review C2). Audit first, membership
      // last — the transaction is atomic either way.
      await audit(tx, teamId, actor.id, selfLeave ? "member.left" : "member.removed", {
        user: targetId,
      });
      await tx.query(
  ...
  app.post("/v1/teams/:team/invitations", async (c) => {
    ...
    const role = parse(TeamRoleSchema.exclude(["owner"]).default("member"), body.role ?? "member");
    ...
    const result = await withUserTx(pool, user.id, async (tx) => {
      ...
      await audit(tx, teamId, user.id, "member.invited", { email: emailAddr, role });
      return {
  ...
  app.delete("/v1/teams/:team/invitations/:id", async (c) => {
    ...
    await withUserTx(pool, user.id, async (tx) => {
      ...
      if (!r.rowCount) throw new HttpError(404, "not_found", "Invitation not found");
      await audit(tx, teamId, user.id, "invitation.revoked", { invitation: inviteId });
    });
    return c.json({ ok: true });
  });
  ...
  app.post("/v1/invitations/accept", rateLimit("invite-accept", 30, 10 * 60_000));
  app.post("/v1/invitations/accept", async (c) => {
    ...
    const joined = await withSystemTx(pool, async (tx) => {
      ...
      if (!invite) throw new HttpError(404, "invalid_invite", "This invite link is no longer valid");
      if (invite.email.toLowerCase() !== user.email.toLowerCase()) {
        throw new HttpError(403, "wrong_account", `This invite was sent to ${maskEmail(invite.email)}`);
      }
      await tx.query(
      ...
      await audit(tx, invite.team_id, user.id, "member.joined", { invitation: invite.id });
  ...
  // ── Team settings doc (O4) — the engine's team layer ────
