# web-app — `zeros-web` (unified Cloudflare Pages)
...
| **`zeros.build`** (+ `www`, `zeros.design`) | Marketing SPA (Vite/React from `website/marketing`) |
...
`website/marketing/pnpm-lock.yaml` (committed), so the CF build never downloads
...
After changing `website/marketing/package.json`, regenerate that lockfile:
...
| SPA `_redirects` | Explicit paths only (`/changelog`) — not `/*`; keep in sync with marketing `src/routes.tsx` |
