...
/** Marketing client routes that must fall back to index.html (React Router).
 *  KEEP IN SYNC with website/marketing/src/routes.tsx and the SPA_REDIRECTS
...
async function fetchMarketingAsset(env: Env, request: Request, url: URL): Promise<Response> {
  const path = url.pathname.replace(/\/$/, "") || "/";
  // Known React Router paths → always serve index.html (client router takes over).
