...
export function marketingOrigin(env: Pick<Env, "MARKETING_ORIGIN"> | Env): string {
...
export function marketingHosts(env: Pick<Env, "MARKETING_HOSTS"> | Env): string[] {
  return splitHosts(env.MARKETING_HOSTS, DEFAULT_MARKETING_HOSTS);
}

export function appHosts(env: Pick<Env, "APP_ORIGIN" | "APP_HOSTS"> | Env): string[] {
...
/** Marketing CSP — allows Google Fonts used by website/marketing/index.html. */
...
export function applyHostHeaders(res: Response, kind: HostKind): Response {
  ...
  headers.set("Content-Security-Policy", kind === "marketing" ? MARKETING_CSP : APP_CSP);
