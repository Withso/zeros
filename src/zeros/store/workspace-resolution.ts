...
// ──────────────────────────────────────────────────────────
//
// Shared helper used by every Column 2 surface (top bar, chat-tabs
...
/** Resolve the exact folder allowed to own Column 2's first paint.
 *
 ...
 *
 *  On the WEB build the spawn MUST carry a workspaceId (the engine rejects a
 *  remote spawn with a bare host path), so the bridge workspace list is passed
 *  here to cover `local-main` + every managed workspace. */
