// ──────────────────────────────────────────────────────────
// Projects store — Column 1's top-level entity (roadmap 03a UI)
// ──────────────────────────────────────────────────────────
//
...
// pattern in src/native/recent-projects.ts so the two stay in sync —
...
// ── Engine write-through (Phase 1b) ──────────────────────────
//
// localStorage stays the desktop's boot cache, but the engine's `repos` table
// is the source of truth that web/mobile read over the bridge. So every mutation
// is mirrored to the engine, keyed on repoRoot. Best-effort + fire-and-forget:
 ...
 *  when adding a project. Keep in sync with src/engine/git/repo.ts. */
...
// re-exported here so existing importers (column1.tsx) keep their path. A
...
export function pruneWorktreePhantomProjects(): number {
  ...
  if (pruned.length > 0) {
    ...
    // canonical repos table — leaving them there means web/mobile keep showing
    // them. Best-effort (no-op without a bridge); the engine ALSO prunes them
