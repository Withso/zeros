...
// launch from the column-3 terminal panel.
//
...
// (claude / codex / cursor-agent / opencode carry a logo; the rest
// are name-only). "+ Add agent" lets the user register any CLI by its
...
export function TerminalAgentsSection() {
  ...
  // Core agents (claude / codex / cursor-agent / opencode) are always shown;
  // other built-ins (amp / antigravity / droid) show until the user removes
  // them; custom agents append after. Saved edits overlay the built-in seed.
      ...
      >
        {agents.map((agent) => {
          ...
          return (
            ...
            >
              {/* Logo only for agents that carry one (claude / codex /
                  cursor-agent / opencode); every other agent is name-only. */}
              {agent.icon && (
                <AgentIcon
                  agentId={agent.id}
                  iconUrl={agent.icon}
...
function TerminalAgentCard({
  ...
  agent: TerminalAgent;
  mode: "edit" | "new";
  /** Core agents (claude/codex/cursor-agent/opencode) — launch command locked,
   *  not removable. */
