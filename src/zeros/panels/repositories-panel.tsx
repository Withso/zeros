...
//
// Settings foundation Phase 1-3: TOML-backed + flat (no cards) + inheritance.
...
import { CodeTextarea, Tooltip } from "@/zeros/ui/primitives";
import { HighlightedCode } from "../agent/renderers/highlighted-code";
import { toast } from "../ui/primitives/elements";
import { cn } from "@/zeros/ui/cn";
import { removeProject, type Project } from "../store/projects-store";
import {
  clearRepoSettings,
  DEFAULT_BASE_BRANCH,
...
import {
  CHATS_BACKUP_KEY,
  CHATS_TOMBSTONE_KEY,
} from "../store/chats-local-cache";
import { dbDeleteChat } from "../agent/agent-history-client";
import { setSetting } from "../../native/settings";
import { clearChangesFilters } from "../../shell/column3-tabs/changes-filter-store";
import { forgetChangesSnapshots } from "../../shell/column3-tabs/changes-snapshot-cache";
import { clearTerminalFolders } from "../../shell/terminal/terminal-store";
import { clearChatPaneFolders } from "../store/chat-panes-store";
import { clearDashboardRepoFilter } from "../store/dashboard-preferences";
import { folderIsOwnedByProject } from "../store/workspace-resolution";
import {
  isInheritedSource,
  ResetToInherited,
  SettingsActions,
...
import { getProviderPrefs, PROVIDER_KEY_ENV_VARS } from "./provider-prefs";
import { resetEngineToDefault } from "../../native/native";
 ...
 *  files still carry after the 2026-07-17 slimming (scripts + the git/prompts
...
/** The card every variable list renders in — the single-row reference design
 *  (founder direction 2026-07-16): lifted `bg1-highlight` fill, `border1`
...
// (founder direction 2026-07-16, replacing the freetext Remote / Base-branch
// inputs):
...
//     and refs are left intact too. (Engine: src/engine/git/worktree.ts.)
  ...
  const handleRemove = async () => {
    ...
    try {
      ...
      if (
        ...
        // dead Column 2. Otherwise pin the scope and let the tab strip's
