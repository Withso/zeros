...
//
// API keys themselves stay in the macOS keychain (src/native/
...
import { getSecret, SECRET_ACCOUNTS } from "../../native/secrets";
...
/** Settings foundation Phase 1: mirror provider choices into the engine-owned
 *  ~/.zeros/settings.toml ([providers.<agentId>]) so the user file is the
 *  durable record. localStorage stays the synchronous read cache until the
 *  spawn path moves onto resolved settings (Phase 4). Fire-and-forget — a
