...
import { parseInviteToken } from "../team/invite-link";
import { requestTeamResync } from "../team/team-sync";
import { setActiveTeamId } from "../team/active-team";
import { refreshTeams, useActiveTeam, useTeams } from "../team/team-store";
...
function MembersBody({
   ...
   *  keeps offering admin controls the server will now reject (M4). */
...
function InviteComposer({
  ...
  const send = async () => {
    ...
    for (const email of parsed.valid) {
      try {
        const { invitation } = await controlPlane.createInvitation(teamId, email, role);
        sent.push(email);
  ...
  const remove = async () => {
    ...
    try {
      ...
      if (isSelf) {
        ...
        // team would 404 into a spurious error toast (review M2); the
        // active-team change remounts/unmounts MembersBody with fresh data.
        await refreshTeams();
        requestTeamResync();
      } else {
        await onChanged();
