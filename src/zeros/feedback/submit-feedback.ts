...
// (packages/feedback-intercom-webhook) turns it into an Intercom conversation
...
import { analyticsDistinctId, capture } from "@/zeros/analytics/posthog";
import { getSession } from "@/zeros/auth/auth-store";
...
export interface FeedbackInput {
   ...
   *  — see electron/ipc/commands/logs.ts). Only set when the user checked
 ...
 *  (electron/ipc/commands/logs.ts MAX_EXPORT_CHARS), so View is byte-identical
