...
// boundary between this chat window's pane and everything outside it (column 3,
...
describe("isFocusHeldElsewhere", () => {
  it("holds for an engaged input/menu/dialog regardless of pane", () => {
    ...
    // A column-3 CodeMirror editor reads as contenteditable → held.
    expect(
      isFocusHeldElsewhere({
  ...
  it("leaves a plain element focused OUTSIDE this window (column 3 / other pane)", () => {
    // The decisive rule: column 3's file-tree host, Changes buttons, and Review
    ...
    const col3Button = mockEl({ tagName: "BUTTON" });
    const fileTreeHost = mockEl({ tagName: "FILE-TREE-CONTAINER" });
    expect(
      isFocusHeldElsewhere({
        activeElement: col3Button,
...
describe("shouldReclaimComposerFocus", () => {
  ...
  it("stands down for a click outside this window (column 3 / another pane)", () => {
    expect(
  ...
  it("does not steal when a within-pane click opened a column-3 surface", () => {
    ...
    const col3Target = mockEl({ tagName: "BUTTON" });
    expect(
      shouldReclaimComposerFocus({
        ...base,
        activeElement: col3Target,
        paneRoot: mockPaneRoot([]), // does not contain col3Target
      }),
    ).toBe(false);
  });
