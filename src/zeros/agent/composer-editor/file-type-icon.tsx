...
// ──────────────────────────────────────────────────────────
//
// User spec (2026-06-08): the @-mention pill must use "the same icon from
// the file tab". The Files tab wraps @pierre/trees with its bundled
...
// FLAG (RULES.md §2.2 library boundary): @pierre's "complete" icon palette
...
export function FileTypeIcon({
  ...
  if (kind === "folder") {
    return (
      <Folder size={size} className={cn("shrink-0 text-fg2", className)} />
    );
  }
  if (kind === "selection") {
    return (
      <MousePointer2
        size={size}
        className={cn("shrink-0 text-fg2", className)}
      />
    );
  }
