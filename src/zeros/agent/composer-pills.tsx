...
// in src/zeros/lib/ rather than redefining it inline here.
...
// 01s (2026-05-20): pill label text-xs → text-xs (12px). User
// reported pills read too small against the text-sm (14px) textarea
// body. text-xs sits one step under body — chrome should read one
...
// 2026-07-12 (user spec): composer toolbar pills sit at a 6px radius —
...
// (TOOLBAR_CARET was removed 2026-07-10 — the model pill's ChevronDown was
// its last consumer; the pill is now just logo + label per user spec.)
...
// ── ModelPill ────────────────────────────────────────────
//
// 2026-07-10 spec: the pill opens the UNIFIED agent+model dropdown
...
export function ModelPill({
  ...
  return (
    ...
    >
      {/* 2026-07-10 (user spec): 12px logo, and no ChevronDown caret — the
          pill is just logo + label. */}
...
// 2026-07-08 (user spec): stripped to just the text — no resting background and
// no battery/charge-fill indicator (both removed). Label color is --fg2,
...
// the "+" → Permissions submenu (2026-07-10 spec). Icon-only at rest: no
...
export function PermissionToggle({
  ...
  // remount + replay both in lockstep (uniform motion, per the 2026-07-10
  // follow-up spec). Each click also (re)arms the 3s hide timer on the label.
  ...
  return (
    ...
    // the name flashes must NOT cycle the mode (user spec).
    <span className="inline-flex items-center">
      ...
      {/* The picked mode's name, flashed to the RIGHT of the icon for 3s after
          ...
          through the delay) so it trails the icon just slightly (user spec).
...
//
// 2026-06-08 (user spec): in the guarded permission modes (Claude Plan, Codex
// Ask for approval, Cursor Ask — see permissionModeShowsFrame) the composer
...
// `3 5` paints a 3-long rounded dash with a 5 gap (2026-06-09 user spec — the
// near-zero `0.01` dot looked too small). The svg is inset 1px so we avoid
