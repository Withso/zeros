// ──────────────────────────────────────────────────────────
// AgentModelMenu — the unified agent + model dropdown (2026-07-10 spec)
// ──────────────────────────────────────────────────────────
//
// One dropdown for BOTH the chat composer's ModelPill and the new-workspace
// dispatcher's picker. Layout (the user's reference design):
//
//   ┌──────┬──────────────────────────────────┐
//   │ [C]  │  🔍 Search models…               │
//   │ [O]  │  Opus 4.8  ✓        ★   ⌘1  │
...
//   └──────┴──────────────────────────────────┘
//
//   - Left rail: one BRAND-COLORED logo per runnable agent (2026-07-10
//     feedback: real colors, no monochrome; the ★ favorites tab was
...
//     trailing ⌘N hint.
//   - UNIVERSAL SEARCH (2026-07-10 feedback): the search box auto-focuses
...
//     the search (2026-07-10 follow-up: digits used to switch rails,
//     which ate the leading "1" of a "1M" search).
...
//     that agent).
//   - `redirectCrossAgent` (2026-07-10 spec): once a chat has its first
...
/** Rail-tab order = the plain-digit shortcut order the spec names:
...
export function AgentModelMenu({
  ...
  // chat's current agent with a clean query (the spec's "whichever agent is
  // selected in the composer … will be shown"). The tab effect also keys on
  ...
  // tab's (2026-07-10 feedback).
  const searching = search.trim().length > 0;

  // Row-highlight mode (2026-07-10 feedback, refined):
  ...
  const showCmdkSelection = hlMode === "idle" || hlMode === "pointer" || hlMode === "kb";
  ...
  // onKeyDown on the popover wrapper), for two user-reported reasons
  // (2026-07-10):
  ...
  return (
    <Popover open={open} onOpenChange={onOpenChange}>
      ...
      <PopoverContent
        ...
        // the trigger pill (user-reported). Skip the restore — pointer users
        // never noticed it, and keyboard flow resumes in the composer.
          ...
          {!searching && (
            ...
            >
              {groups.map((g) => (
                ...
                >
                  {/* Real brand colors (2026-07-10 feedback) — no monochrome.
                      16px like every other logo in this dropdown (2026-07-15). */}
                  <AgentIcon
                    agentId={g.agent.id}
              ...
              {rows.map((row, i) => {
                ...
                return (
                  <CommandItem
                    ...
                    onSelect={() => pick(row)}
                  >
                    {/* Single-line row: just the model name (2026-07-10
                        feedback — no agent subtitle, no NEW badge). While
                        SEARCHING the rows span every agent and the rail is
                        hidden, so each row leads with its agent's 16px brand
                        logo to keep the results attributable (2026-07-15). */}
                    ...
                    {/* Trailing cluster — ✓ (active model), then ★, ⌘N hint
                        LAST (2026-07-10 feedback: the tick sits immediately
                        left of the star, not after the name). The ★ is this
 ...
 *  rail/model-list divider border (2026-07-10 feedback).
 *
 ...
 *  painting a :focus-visible ring on the first logo the moment a shortcut
 *  key is pressed (user-reported, 2026-07-10). */
function RailButton({
  active,
  label,
  onSelect,
  ...
  return (
    <Tooltip label={label} side="right">
      <button
        ...
        onClick={onSelect}
        className={cn(
          "text-fg2 relative flex size-8 shrink-0 items-center justify-center rounded-sm outline-none transition-[background-color,color] duration-120 ease-out",
          active
