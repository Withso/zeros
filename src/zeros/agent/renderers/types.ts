...
export interface RendererContext {
  ...
  /** Phase 2 chat overhaul (2026-05-07) — click-to-edit on past
   *  user messages. Renderer calls this with `(messageId, editedText)`;
   *  agent-chat truncates SQLite + the in-memory store at messageId
   *  (inclusive) and dispatches editedText as a fresh user prompt.
   *  Files on disk are NOT reverted — the user explicitly described
   *  this as "editing and continuing, no code changes are reverted."
   *  See docs/research/08_revert_capability_per_agent.md for the
   *  full revert plan (snapshot-based) which is the follow-up.
   *
   *  2026-06-09: edit mode reconstructs the WHOLE message inline in the
   ...
   *  surfaces with no row-1 (e.g. the empty-state composer). */
...
export interface RendererRegistry {
  ...
  /** Stage 4.4 — non-text/non-tool message kinds (mode_switch,
