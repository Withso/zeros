// ──────────────────────────────────────────────────────────
// CompactionRecordCard — the compaction tool-call row (§3.5 A/B/C)
// ──────────────────────────────────────────────────────────
//
// Context compaction renders like any other thing the agent does — a line
// in the working stripe with EXACTLY TWO states (2026-07-12 design rev 2):
//
 ...
 *  per the 2026-07-12 spec ("a pill with no icon: Done"). */
function DoneChip() {
  return (
    <span className="border-border1 text-fg2 flex shrink-0 items-center rounded-md border px-1.5 py-0.5 font-mono text-xxs tracking-wide uppercase">
      Done
    </span>
  );
}
...
function readDetail(input: unknown): { trigger?: string; preTokens?: number } | null {
