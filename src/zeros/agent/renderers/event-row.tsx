...
// ──────────────────────────────────────────────────────────
//
// Roadmap 01f (2026-05-19). Historically this collapsed the
// per-tool card variants into ONE inline row.
// inline row. Borderless, no card chrome. The row is a 20px
...
// an outer-wrapper gap (user feedback 2026-06-19).
//
...
// 01g: status conveyed by row tint, not a right-side text badge.
// Per the user's feedback ("no right-side tag like read/exit 0 —
// empty on right"), the row tone is the only status signal.
  ...
  return (
    <div className="flex flex-col">
      ...
      {expandable && open && (
        ...
        // body fills the lane width (user feedback). The detail's own box
