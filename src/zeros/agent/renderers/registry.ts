...
// ModeSwitchBanner) stay on the flat row — see roadmap §0.
...
export const defaultRegistry: RendererRegistry = {
  ...
  toolByKind: {
    ...
    // surface is the composer-slot <QuestionCard> (src/zeros/agent/question-
    // card.tsx), driven by the blocking QuestionRequest — not this card.
    question: QuestionRecordCard,
    // Context compaction (§3.5) — two-state row: spinning "Compacting.." →
    // "Context compacted" + Done chip.
    compaction: CompactionRecordCard,
    // §3.6 R2 — the turn continued on a fallback model. Durable, expandable
...
export function resolveRenderer(
  ...
  // (error_notice today; plan / question / subagent arrive in later
  // slices).
  const byKind = (
