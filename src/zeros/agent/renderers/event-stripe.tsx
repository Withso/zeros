...
// Brightness split inside the feed (globals.css, `.zeros-working-feed`):
...
// markdown at every depth (user feedback 2026-06-19). The working-vs-answer
...
export const EventStripe = memo(function EventStripe({
  ...
  // collapsed summary chip while the user's decision pends (user feedback
  // 2026-07-04: the plan must stay visible until approved/rejected).
  ...
  // working feed reads uniformly (user feedback 2026-07-06 — a one-edit turn
  // rendered a bare Edit row while multi-event turns got the group chip).
  const showHeader =
    ...
    <div className="flex flex-col">
      {showHeader && (
        <button
          ...
          // right (user feedback — hover should fit the content).
          className="group/event-stripe -ml-1 flex w-fit min-w-0 max-w-full items-center gap-2 rounded-md px-1 py-1 text-left transition-colors hover:bg-bg2-hover/40"
        ...
        >
          ...
          {/* Activity icons at the END, after the text (2026-06-18, per user
              feedback) — deduped tool kinds + Bot (any sub-agent) + Brain (any
              reasoning), capped at 5, visible. */}
          <StripeIcons events={events} />
        </button>
      )}
      {expanded && (
        // `zeros-working-feed` tags the group so globals.css mutes it to fg2 and
        ...
        // its many tool calls tight — no gap — exactly as before (user feedback
        // 2026-06-19). Top-level entries are wrapped in a plain div so the gap
  ...
  return (
    <div className="flex shrink-0 items-center gap-1" aria-hidden="true">
      {icons.map((Icon, i) => (
        <span
          key={i}
          className="inline-flex size-3 items-center justify-center text-fg2"
        >
