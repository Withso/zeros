...
// ──────────────────────────────────────────────────────────
//
// Roadmap 01f (2026-05-19). Replaces the per-tool variant
// headers (tool-read, tool-edit, tool-shell, …) with one
...
export function metaForEvent(message: AgentMessage): EventMeta {
  ...
  if (message.kind === "text" && (message as any).role === "thought") {
    ...
    return {
      Icon: Brain,
      label: "Thinking",
      // Glimpse of the thought as the collapsed-row preview (user feedback):
      ...
      // Char count intentionally omitted from the right edge (user
      // feedback). The thought text is still inspectable via expand.
      trailing: undefined,
      expandable: chars > 0,
    };
  }
  if (message.kind === "error_notice") {
    ...
    // into the agent's answer (user spec 2026-07-04).
    const m = message as any;
    const text = (m.message as string) ?? "";
    // An api_retry burst reads as "Reconnecting agent", not a warning (user
    // spec 2026-07-10): while live it renders as a shimmer row (see
...
function metaForTool(tool: AgentToolMessage): EventMeta {
  ...
  if (kind === "read") {
    ...
    // Count in the LABEL ("Read 403 lines" / "Read image"), filename as the tag
    // (user spec 2026-06-19). Consistent across all adapters.
    return {
      Icon: FileText,
      label: image
        ? "Read image"
