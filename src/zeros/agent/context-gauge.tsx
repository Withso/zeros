// ──────────────────────────────────────────────────────────
// ContextGauge — the context ring + breakdown popover (§3.5 A/B/C)
// ──────────────────────────────────────────────────────────
//
// One small ring beside Send, in every chat. It quietly fills as the
// conversation grows and has EXACTLY TWO visual states (2026-07-12 design
// rev 2): calm `fg2` chrome its whole life, `red-primary` at ≥90% — the
...
// ring renders DISABLED (empty track) and the popover says so plainly
// (user decision 2026-07-12: option (b), never a made-up number).
//
// The ring is ALWAYS present (user spec 2026-07-12 rev 3): before the first
...
function Ring({ fraction, disabled }: { fraction: number; disabled?: boolean }) {
...
): usage is { size: number; used: number; categories?: AgentUsage["categories"] } {
  return (
    !!usage &&
  ...
  return (
    <Popover>
      ...
      <PopoverContent align="end" side="top" className="p-3.5">
        ...
        message — no "Context" title, no bar, no footer (user spec
        2026-07-12). */}
        {data ? (
          <>
