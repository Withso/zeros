...
export function SlashCommandPicker({
  ...
  // (user spec 2026-06-08). bg1 background per the same spec.
  const menuWrap =
  ...
  // the border-b) with the tab chips vertically centered (user spec 2026-06-25).
  const tabBar = (
        ...
        <div ref={listRef} className="max-h-[220px] overflow-y-auto">
          {visible.map((cmd, i) => {
            ...
            return (
              ...
              >
                {/* Text "/" marker in place of an icon (user spec — the lucide
                    Slash glyph read poorly). The command name drops its own
                ...
                {/* One line: name then the description inline (user spec). */}
                ...
                {/* Skill badge — bordered bg1 chip at the row's end so the user
                    can tell a skill from a command at a glance (mirrors the
                    "takes input" affordance; user spec 2026-06-25). */}
                {isSkill && (
