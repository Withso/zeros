...
// bootstrap the store needs at module load, and is reachable on every client
// (web/cloud included) — unlike the old electron/db.ts backstop, which Phase 2c
// dropped. Policies are a per-device convenience; cross-device sync (via the
// engine) is a clean future move, isolated to this module + the store mutators.
// ──────────────────────────────────────────────────────────
