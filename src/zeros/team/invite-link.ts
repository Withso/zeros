// ──────────────────────────────────────────────────────────
// Invite-link plumbing (O3 accept flow, docs/teams.md Part F)
//
...
import { onDeepLink } from "../../native/native";
...
export function parseInviteToken(raw: string): string | null {
  ...
  const isInviteRoute =
    ...
    // random webpage's https link doesn't parse as an invite (audit L3).
    (url.protocol === "https:" &&
 ...
 *  being dropped because no listener existed yet (audit M3). `onInvite`
