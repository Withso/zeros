...
import { getSettingMigrated, setSetting } from "../../native/settings";
import {
  CONTROL_PLANE_URL,
  ControlPlaneError,
  controlPlane,
...
import { getActiveTeamId, setActiveTeamId, subscribeActiveTeam } from "./active-team";
...
// teams after the store was cleared for account B (review M1).
let generation = 0;
...
// until the first fetch confirms (review L1). Advisory only — the live
// store state wins the moment it's ready.
 ...
 *  the Administration tabs flash in and vanish — exactly what L1 fixed. */
...
export function useTeams(): TeamStoreState & { reload: () => Promise<Me | null> } {
