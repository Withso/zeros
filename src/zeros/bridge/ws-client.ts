...
import { PROTOCOL_VERSION } from "@zeros/core/version";
import {
...
import { getAuthAccessToken } from "../auth/auth-token";
import { CHANNEL, type Channel } from "../flags";
...
/**
 * Resolve the loopback /ws auth token (C1). The engine requires it on the
...
function resolveEngineToken(): Promise<string> {
  ...
  cachedTokenPromise = (async () => {
    ...
    try {
      const { isNativeRuntime, nativeInvoke } =
        await import("../../native/runtime");
      if (isNativeRuntime()) {
...
export class RuntimeClient {
  ...
  async connect(): Promise<void> {
    ...
    // C1: present the per-launch token on the upgrade. Omitted only when the
  ...
  private onTransportOpen(): void {
    ...
    //   • RELAY (web/phone/other machine): the engine requires the token to
