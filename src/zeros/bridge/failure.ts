...
/** Phase 2 chat overhaul (2026-05-07): mirrors SESSION_EXPIRED_KEYWORDS
 *  in src/engine/agents/adapters/shared/session-expiry.ts. The engine usually classifies
 ...
 *
 *  2026-05-28: broadened to cover the codex adapter's user-facing
 *  wording ("no longer has a rollout", "lost the rollout") plus the
 *  generic snippets from t3code's CodexSessionRuntime. Keep in sync
 ...
 *
 *  Phase 2 chat overhaul (2026-05-07): `session-expired` is now in the
 *  recoverable set. When the vendor's persisted rollout/session is
 ...
 *  + force:true`. Pattern B from docs/research/09_session_resume_patterns.md.
 *  The rebuild discards the stale id; the next CLI spawn omits
 *  `--resume`, starts cold. We don't replay history into the fresh
 *  session yet — that's Phase B2 follow-up. The user keeps their
 *  visible chat history (it's in our SQLite) and sees a small
