...
//   1. initializes PostHog (no-op on web / opted-out / no-key),
...
export function AnalyticsBoot() {
  ...
  // Native crash forwarding: the Electron main process (electron/main.ts) and the
  // engine supervisor (electron/sidecar.ts) push crashes over IPC so PostHog
  // error tracking sees faults that happen OUTSIDE the renderer. Metadata +
  // app-internal stacks only — never user content. No-ops on web/browser
  // (nativeListen returns a no-op unsubscribe there).
  useEffect(() => {
