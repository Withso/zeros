...
// ──────────────────────────────────────────────────────────
//
// Replaces the old SDK-in-the-renderer model. The main process owns the
// full token pair (electron/ipc/commands/auth-session.ts) — this module is a
// thin, offline-tolerant mirror: it asks main for "a live access token" or "who's
// signed in" and never sees a refresh token at all. Desktop-only by design (the
// product is desktop-only per the 2026-06-25 web-app removal; there is no
// browser fallback here, matching that decision).
// ──────────────────────────────────────────────────────────
...
export async function getSession(): Promise<AuthSessionInfo | null> {
  ...
  return tokenRes?.access_token && user ? { access_token: tokenRes.access_token, user } : null;
}
