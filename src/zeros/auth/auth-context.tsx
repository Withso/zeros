...
// ──────────────────────────────────────────────────────────
//
// The main process owns the full Auth0 token pair (electron/ipc/commands/
// auth-session.ts); this provider is a thin, offline-tolerant mirror over IPC —
// it never sees a refresh token. Desktop-only (no browser fallback; matches the
// 2026-06-25 web-app removal).
//
// Lifecycle notes:
...
function AuthProviderInner({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    let offStateChange: (() => void) | undefined;
    ...
    offStateChange = onAuthStateChange((next) => apply(next));
    ...
    void nativeListen<{ ticket?: string; nonce?: string }>(
      ...
      async ({ ticket, nonce }) => {
        ...
        // enters this renderer heap (H1). The browser never held our session:
...
async function resolveDesktopScheme(): Promise<DeepLinkScheme> {
  ...
  try {
    ...
    // value electron/deep-link.ts registered + validates against, so the scheme
