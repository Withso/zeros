...
// ──────────────────────────────────────────────────────────
//
// Roadmap 03b iframe migration. Replaces the WebContentsView-based
...
//     parent React Flow canvas (Phase 5) "just works" because
...
// electron/ipc/iframe-session.ts.
//
...
import { isLoopbackUrl } from "../../shell/column3-tabs/localhost-url";
 ...
 *  picker captured it. Stage 2 wires this up via postMessage. */
...
export interface UseIframeWebviewResult {
  ...
  // ── Design Mode (Stage 2 of iframe migration) ──────────
...
export function useIframeWebview(
   ...
   *  H16: the picker shares contentWindow with the (untrusted) visited page, so
  ...
  useEffect(() => {
    ...
    const onMessage = async (ev: MessageEvent) => {
      ...
      if (op === "single-selected") {
        ...
        if (!isValidSelected(payload)) return; // H16: drop a malformed/forged selection
        ...
        if (enriched.altKey) {
          ...
          // composer can pick it up directly (matches Phase 4.5
          // behavior). Don't add to selectedElements — alt-click
          // is a separate flow.
      ...
      if (op === "batch-selected") {
        ...
        // H16: validate the batch (bounded length + every element well-formed).
