// ──────────────────────────────────────────────────────────
// Settings foundation — agent spawn env composition (Phase 4)
// ──────────────────────────────────────────────────────────
//
...
const HAZARD_REASON: Record<SpawnEnvHazard, string> = {
  ...
  "credential-redirect": "credential-redirect guard — would re-route the agent's API traffic/credentials",
  "secret-shaped": "secret-shaped name — keep credentials in the Keychain, not the env table",
};
...
export function parseDotenv(text: string): Record<string, string> {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const body = /^export\s+/.test(line) ? line.replace(/^export\s+/, "") : line;
...
export function resolveSpawnEnv(cwd: string, mainRepoRoot?: string): SpawnEnvResult {
  ...
  try {
    ...
    return { env: out, warnings: [`settings resolve failed: ${String((err as Error)?.message ?? err)}`] };
  }
  ...
  if (isPlainObject(table)) {
    for (const [k, v] of Object.entries(table)) {
      ...
      if (hazard && !hazardAllowedFromLayer(hazard, sources[`env.${k}`])) {
        warnings.push(`env: ignored unsafe variable name "${k}" (${HAZARD_REASON[hazard]})`);
        continue;
      }
      out[k] = v;
    }
  ...
  if (Array.isArray(files)) {
    ...
    for (const f of files) {
      ...
      if (path.isAbsolute(f) || f.split(/[\\/]/).includes("..")) {
        warnings.push(`env_file rejected (must be a relative path inside the repo): ${f}`);
        continue;
      }
      ...
      try {
        ...
        if (st.size > ENV_FILE_MAX_BYTES) {
          warnings.push(`env_file too large (> ${ENV_FILE_MAX_BYTES} bytes), skipped: ${f}`);
          continue;
        }
      } catch {
      ...
      try {
        for (const [k, v] of Object.entries(parseDotenv(text))) {
          const hazard = spawnEnvNameHazard(k);
          if (hazard) {
            warnings.push(`env_file "${f}": ignored unsafe variable name "${k}" (${HAZARD_REASON[hazard]})`);
            continue;
          }
          out[k] = v;
        }
...
export function mergeSpawnEnv(
  ...
  for (const w of warnings) console.warn(`[agents] settings env (${cwd}): ${w}`);
