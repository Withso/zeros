// ──────────────────────────────────────────────────────────
// Settings foundation — bridge op implementations (Phase 1)
// ──────────────────────────────────────────────────────────
//
...
//
// Remote policy (plan §4.3): resolve/read/write are all open to paired
...
export const READABLE_LAYERS = ["user", "managed", "repo", "repo-local", "workspace-local"] as const;
export const WRITABLE_LAYERS = ["user", "repo", "repo-local", "workspace-local"] as const;
...
function layerPath(layer: ReadableLayer, repoRoot?: string): string {
  switch (layer) {
    ...
    case "workspace-local": {
      ...
      return layer === "repo" ? repoSettingsPath(repoRoot) : repoLocalSettingsPath(repoRoot);
    }
  }
}
...
export function opSettingsRead(layer: ReadableLayer, repoRoot?: string): SettingsReadOpResult {
  const filePath = layerPath(layer, repoRoot);
  return { layer, path: filePath, ...readSettingsFile(filePath) };
}

export function opSettingsResolve(repoRoot?: string, mainRepoRoot?: string): ResolvedSettings {
  ...
  const repo = repoRoot ? readSettingsFile(repoSettingsPath(repoRoot)) : undefined;
  const repoLocalRoot = mainRepoRoot ?? repoRoot;
  const repoLocal = repoLocalRoot ? readSettingsFile(repoLocalSettingsPath(repoLocalRoot)) : undefined;
  const isDistinctWorktree =
    !!repoRoot && !!mainRepoRoot && path.resolve(mainRepoRoot) !== path.resolve(repoRoot);
  ...
  for (const [name, r] of [
    ...
    if (r?.error) resolved.warnings.push(`${name}: file is malformed and was ignored (${r.error})`);
  }
  return resolved;
}
...
function containsRedactedSentinel(value: unknown): boolean {
  ...
  if (isPlainObject(value)) return Object.values(value).some(containsRedactedSentinel);
  return false;
}
...
function redactMcpTable(mcp: unknown): unknown {
  if (!isPlainObject(mcp) || !Array.isArray((mcp as { servers?: unknown }).servers)) return mcp;
  const servers = (mcp as { servers: unknown[] }).servers.map((s) => {
    ...
    for (const field of ["env", "headers"] as const) {
      ...
      if (isPlainObject(tbl)) {
        out[field] = Object.fromEntries(Object.keys(tbl).map((k) => [k, REDACTED_SENTINEL]));
      }
    }
    return out;
  });
...
export function redactDocForRemote(doc: RawSettingsDoc): RawSettingsDoc {
  ...
  if (isPlainObject(doc.env)) out = { ...out, env: redactEnvTable(doc.env) as RawSettingsDoc };
  if (doc.mcp !== undefined) out = { ...out, mcp: redactMcpTable(doc.mcp) } as RawSettingsDoc;
  return out;
}
...
export function redactResolvedForRemote(resolved: ResolvedSettings): ResolvedSettings {
...
export function opSettingsWrite(
  ...
  if (!isPlainObject(patch)) {
    throw new SettingsOpError("SETTINGS_BAD_PATCH", "settings.write requires a patch object");
  }
  ...
  const filePath = layerPath(layer, repoRoot);
  const schemaUrl = layer === "user" ? SCHEMA_URL_USER : layer === "repo" ? SCHEMA_URL_REPO : null;
...
export function opSettingsWriteRaw(
  ...
  if (typeof text !== "string") {
    throw new SettingsOpError("SETTINGS_BAD_PATCH", "settings.writeRaw requires a text string");
  }
...
export function ensureLocalSettingsIgnored(repoRoot: string): void {
  ...
  try {
    ...
    if (lines.includes(IGNORE_LINE) || lines.includes(`/${IGNORE_LINE}`) || lines.includes(".zeros/")) {
      return;
    }
 ...
 *  D7 mapping: runOnCreate scripts join into `scripts.setup` (`&&`); the
 *  first non-runOnCreate script becomes `scripts.run`; leftovers are
 *  reported, not silently dropped. LOCAL-ONLY op. */
export function opSettingsMigrateLegacy(input: MigrateLegacyInput): MigrateLegacyResult {
  ...
  for (const entry of input.repos ?? []) {
    ...
    const setupCmds = legacyScripts
      .filter((s) => s?.runOnCreate && typeof s.command === "string" && s.command.trim())
      .map((s) => (s.command as string).trim());
    if (setupCmds.length > 0) scripts.setup = setupCmds.join(" && ");
    const runCandidates = legacyScripts.filter(
      (s) => !s?.runOnCreate && typeof s?.command === "string" && s.command.trim(),
    );
    if (runCandidates.length > 0) scripts.run = (runCandidates[0].command as string).trim();
    ...
    if (typeof legacy.workspacesPath === "string" && legacy.workspacesPath.trim()) {
      writeMergeUnder("repo-local", { workspaces: { path: legacy.workspacesPath.trim() } }, repoRoot);
  ...
  for (const [agentId, prefs] of Object.entries(input.providers ?? {})) {
    ...
    if (prefs.authMethod === "apiKey" || prefs.authMethod === "api-key") p.auth = "api-key";
...
function writeMergeUnder(layer: WritableLayer, patch: RawSettingsDoc, repoRoot?: string): void {
...
function dropExisting(patch: RawSettingsDoc, existing: RawSettingsDoc): RawSettingsDoc {
