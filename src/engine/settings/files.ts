...
// ──────────────────────────────────────────────────────────
//
// The ONLY module that touches settings files on disk; renderer and web
// clients go through the bridge ops (Phase 1). Locations (plan §4.1):
//
...
// are atomic (tmp + rename, same dir). (Supersedes decision D10, which accepted
// comment loss.)
// ──────────────────────────────────────────────────────────
...
import { SCHEMA_URL_REPO, SCHEMA_URL_USER, type RawSettingsDoc } from "./schema";
...
export function userSettingsDir(): string {
  if (process.env.ZEROS_USER_SETTINGS_DIR) return process.env.ZEROS_USER_SETTINGS_DIR;
...
export function readSettingsFile(filePath: string): ReadSettingsResult {
  ...
  try {
    ...
    if (code === "ENOENT" || code === "ENOTDIR") return { doc: {}, exists: false };
    if (code === "EISDIR") {
      return { doc: {}, exists: true, error: `${filePath} is a directory, not a settings file` };
    }
    return { doc: {}, exists: true, error: String((err as Error).message ?? err) };
  }
...
function serializeSettings(ordered: RawSettingsDoc, existingText?: string): string {
...
function prune(value: unknown): unknown {
  if (Array.isArray(value)) return value.filter((v) => v !== undefined).map(prune);
 ...
 *  Returns a new document (inputs untouched). This is the write-op contract
 *  the bridge exposes in Phase 1. */
export function applySettingsPatch(doc: RawSettingsDoc, patch: RawSettingsDoc): RawSettingsDoc {
  ...
  for (const [key, value] of Object.entries(patch)) {
    ...
    if (isPlainObject(value)) {
      const existing = isPlainObject(out[key]) ? (out[key] as RawSettingsDoc) : {};
...
export function updateSettingsFile(
  ...
  if (current.error) {
    throw new Error(`refusing to overwrite malformed settings file ${filePath}: ${current.error}`);
  }
