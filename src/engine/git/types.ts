...
export interface Workspace {
   ...
   *  no-button decision on cards + the Column-2 header. */
  hasChanges?: boolean;
}
...
export interface CreateWorkspaceOptions {
  ...
  baseBranch?: string;
  /** Original user prompt — used by the background-rename hook (Phase 2)
   ...
   *  env plus ZEROS_* env vars (see setup-hooks.ts). Pattern adopted from
   *  workmux's post_create hook. */
  setupScript?: string;
   ...
   *  against it. Optional. */
  agentId?: string;
  /** H6: allow auto-pickup of a repo-resident `.zeros/setup.sh`. Set only on
