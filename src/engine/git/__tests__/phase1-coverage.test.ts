// Phase 1 — full git command coverage (engine ops).
//   D.1 diff modes (staged / refs / rawPatch) + showCommit
//   D.2 conflicted + conflictState in status
//   D.3 reset / discard / clean
//   D.4 continue / abort
//   D.5 merge / cherry-pick / revert / stash list-apply-drop / deleteBranch / tags
...
async function initRepo(repoRoot: string): Promise<void> {
  ...
  await git(repoRoot, "remote", "add", "origin", "https://example.com/t/p1.git");
...
describe("Phase 1 coverage", () => {
  ...
  beforeEach(async () => {
    workdir = await mkdtemp(path.join(tmpdir(), "zeros-p1-"));
  ...
  // ── D.1 ─────────────────────────────────────────────────
  describe("D.1 diff generalization + showCommit", () => {
    ...
    it("mode refs compares two branches (3-dot)", async () => {
      ...
      const d = await diff({ workspaceId, mode: "refs", base: "main", head: "HEAD" });
  ...
  // ── D.2 + D.4 ───────────────────────────────────────────
  describe("D.2/D.4 conflicts: status + continue/abort", () => {
  ...
  // ── D.3 ─────────────────────────────────────────────────
  describe("D.3 reset / discard / clean", () => {
    ...
    it("discard reverts a tracked working-tree edit", async () => {
      ...
      expect(await readFile(path.join(wsPath, "README.md"), "utf8")).toBe("base\n");
    });

    it("reset --hard requires confirm", async () => {
      await expect(
        reset({ workspaceId, mode: "hard" }),
  ...
  // ── D.5 ─────────────────────────────────────────────────
  describe("D.5 merge / cherry-pick / revert / stash / branch / tags", () => {
  ...
  // ── D.6 — hunk-level apply ───────────────────────────────
  describe("D.6 stage/unstage/discard hunk (git apply)", () => {
    it("stageHunk applies a patch to the index; unstageHunk reverses it", async () => {
      ...
      const d = await diff({ workspaceId, mode: "worktree-vs-index", rawPatch: true });
      expect(d.patch).toContain("hunk line");
      await stageHunk({ workspaceId, patch: d.patch! });
      let s = await status(workspaceId);
      expect(s.staged.find((f) => f.path === "README.md")?.status).toBe("modified");
      expect(s.unstaged).toEqual([]);
      // Reverse it back out of the index.
      const staged = await diff({ workspaceId, mode: "index-vs-head", rawPatch: true });
