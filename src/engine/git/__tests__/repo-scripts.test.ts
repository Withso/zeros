// Phase 4 — repo lifecycle scripts (settings TOML scripts.setup / scripts.archive)
...
async function initRepo(repoRoot: string, settingsToml?: string): Promise<void> {
  await mkdir(repoRoot, { recursive: true });
  await git(repoRoot, "init", "-q", "-b", "main");
  await git(repoRoot, "remote", "add", "origin", "https://example.com/t/scripts.git");
  ...
  if (settingsToml) {
    ...
    await writeFile(path.join(repoRoot, ".zeros", "settings.toml"), settingsToml);
  }
  await git(repoRoot, "add", ".");
  await git(repoRoot, "commit", "-q", "-m", "initial");
}

describe("repo lifecycle scripts (Phase 4)", () => {
  let workdir: string;
  let repoRoot: string;
  let userDir: string;
  ...
  it("runs scripts.archive before the worktree is removed", async () => {
    ...
    const created = await createWorkspace({ repoRoot });
    await archiveWorkspace({ workspaceId: created.workspaceId, stashUncommitted: false });
