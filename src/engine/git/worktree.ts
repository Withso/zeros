 ...
 *  as dead code (C5). `rev-parse --abbrev-ref` returns the literal "HEAD"
...
export interface CreateWorkspaceInput extends CreateWorkspaceOptions {
   ...
   *  repo's committed/working-tree TOML (mirrors the setupScript drop, C1). */
...
async function createWorkspaceInner(
  ...
  // H7: repoSlug is path.join'd into worktreesRoot(). A caller-supplied
  ...
  // H7: baseRef is a bare positional to `git worktree add`. A value starting
...
/** Boot janitor (Phase B retention): drop archive snapshot refs that no longer
 ...
 *  caller can only address repos the owner has opened (mirrors the C1
...
async function archiveWorkspaceInner(
  ...
  // Phase 4: the repo's committed `scripts.archive`, run in the worktree while
 ...
 *  markers behind. Uses `-z` so paths with spaces are byte-exact (C1). */
...
/** Phase 0 — relocate worktrees from the legacy hidden ~/.zeros/worktrees to the
