 ...
 *  Uses `-z` so paths with spaces/special chars are byte-exact (C1). */
...
export async function commit(opts: CommitOptions): Promise<CommitResult> {
  ...
  // C5: read the branch with `rev-parse --abbrev-ref` (returns "HEAD" on
...
export async function stashSave(
  ...
  // M11: serialize push + `rev-parse stash@{0}` per repo so a concurrent stash
