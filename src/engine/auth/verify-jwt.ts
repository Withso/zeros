...
// on any particular identity provider. It was written against Supabase and now
// runs against Auth0 with no code change, which is the clearest evidence of
// that. Do not reintroduce provider-specific names or URL shapes here.
//
...
export function verifyAccountJwt(
  ...
  // M4: an access token MUST carry an expiry — a validly-signed token without
...
export function buildAccountAuthFromEnv(
  ...
  //
  // The origin form used to be SUPABASE_URL and appended `/auth/v1/` — GoTrue's
  // layout, which is wrong for every other issuer including the Auth0 tenant
  // this actually runs against (electron/sidecar.ts passes the full URL, which
  // is why the wrong path never surfaced). `/.well-known/jwks.json` is the
  // RFC 8414 location and is correct for Auth0 and effectively every IdP.
  const jwksUrl =
