...
//
// PHASE 1 (this file) is the spike: a single shared connection token + raised
// keep-alive timeouts (the proxy resets idle upstreams) + server-side keepalive
// pings, surfacing each peer as a `kind: "cloud"` TransportClient so the real
// engine serves the full bridge over it (handshake + file.tree + PTY). Clients
// are ungated by account-binding while `accountAuth` is unset (the spike runs
// without account-binding env), exactly like a trusted local peer.
//
// PHASE 2 layers per-user identity on top: an access token in the WS handshake +
// account/workspace binding, threaded through the engine's existing `client.kind`
// gating mesh (verifyAccountBinding's remote branch). The single-token gate here
// is the seam that grows into that — not a replacement for it.
//
...
export interface CloudTransportOptions {
  ...
  port: number;
  /** Worker-minted connection token presented on the /ws upgrade (via `?token=`
   *  or the `x-zeros-cloud-token` header). Empty string disables the gate —
   *  load-tests / a sandbox already fronted by the preview-token proxy. */
  token?: string;
}
...
export class CloudTransport implements Transport {
  ...
  /** The token gate. An empty configured token disables the check. The token may
   *  arrive as `?token=…` (cleanest for a signed preview URL) or the
   *  `x-zeros-cloud-token` header (when the URL must stay clean). */
