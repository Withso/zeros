...
// ──────────────────────────────────────────────────────────
//
// Before remote workspaces, the engine broadcast every push to every
// client. The moment a second device connects that's a bug: a phone would
// receive the desktop's entire stream. This router fixes it by tracking
...
export class MessageRouter {
   ...
   *  connected client — your Mac AND your web/phone watch the same agent live,
   *  like Paseo (which fans every agent event to all client sessions and has no
   *  per-client session ownership). Access is gated upstream at pairing + the
