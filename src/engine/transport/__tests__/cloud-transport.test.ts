// ──────────────────────────────────────────────────────────
// CloudTransport — the in-sandbox 0.0.0.0 bridge (Phase 1 spike)
// ──────────────────────────────────────────────────────────
//
...
//   • the token gates the /ws upgrade (via `?token=` AND the header), and an
//     empty configured token disables the gate.
...
function attemptUpgrade(
  port: number,
  opts: { path?: string; origin?: string; host?: string; cloudTokenHeader?: string } = {},
...
describe("CloudTransport — /ws token gate (preview proxy is the boundary)", () => {
  it("accepts the right token via ?token=", async () => {
    ...
    await expect(attemptUpgrade(port, { path: `/ws?token=${TOKEN}` })).resolves.toBe("accepted");
  });
  ...
  it("rejects a missing token", async () => {
    const { port } = await startTransport({ token: TOKEN });
    await expect(attemptUpgrade(port, { path: "/ws" })).resolves.toBe("rejected");
  });

  it("rejects a wrong token", async () => {
    const { port } = await startTransport({ token: TOKEN });
    await expect(attemptUpgrade(port, { path: "/ws?token=nope" })).resolves.toBe("rejected");
  });

  it("with no token configured, accepts without a token param", async () => {
    const { port } = await startTransport({});
    await expect(attemptUpgrade(port, { path: "/ws" })).resolves.toBe("accepted");
  });
...
describe("CloudTransport — connected peer is kind:cloud and messages round-trip", () => {
  it("surfaces the client as kind:cloud, delivers onMessage, and broadcasts back", async () => {
    ...
    t.broadcast({ type: "PTY_DATA", source: "engine", sessionId: "s1", data: "hi" } as EngineMessage);
    await new Promise((r) => setTimeout(r, 50));
