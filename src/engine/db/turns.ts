...
// the AUTHORED change set (this turn's own edit/write/delete tool calls — see
// the attribution note in docs/turns-and-reset-to-point-plan-2026-06-24.md), NOT
// a whole-tree diff, so a concurrent chat's edits never leak into this turn.
// ──────────────────────────────────────────────────────────
...
/** §3.6 R6 — one model's share of a turn's bill (the usage popover rows). */
...
/** §3.6 R6 — per-turn token/cost usage, persisted as JSON on the turn row so
...
export interface TurnRow {
  ...
  files: TurnFile[];
  /** §3.6 R6 — tokens/cost for this turn (null when the agent reported none). */
...
export function finishTurn(
  ...
  patch: {
    ...
    postSnapshot: string | null;
    files: TurnFile[];
    /** §3.6 R6 — tokens/cost this turn billed; omitted/null when the agent
