// ──────────────────────────────────────────────────────────
// Legacy agent-history migration (Phase 2c) — one-time, engine-side
// ──────────────────────────────────────────────────────────
//
...
//    Electron sidecar. Absent on web/cloud/standalone → no-op.
// ──────────────────────────────────────────────────────────
...
export function migrateLegacyAgentHistory(): void {
  ...
  try {
    ...
    for (const r of chats) {
      ...
      // M7: import each chat AND its messages atomically. Previously a crash
    ...
    console.warn("[zeros-db] legacy agent-history migration failed (will retry):", err);
  } finally {
    legacy?.close();
  }
}
