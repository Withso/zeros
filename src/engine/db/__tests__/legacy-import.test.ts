...
function makeLegacyDb(): string {
  ...
  ins.run("legacy1", 1, "m1", "text", JSON.stringify({ id: "m1", kind: "text", role: "user", text: "hi from legacy" }), 1);
  ins.run("legacy1", 2, "m2", "text", JSON.stringify({ id: "m2", kind: "text", role: "agent", text: "reply" }), 2);
  db.close();
  return file;
}
...
function chat(id: string, over: Partial<ChatRow> = {}): ChatRow {
  return {
    id, folder: "/p", agentId: null, agentName: null, model: null, effort: "",
    permissionMode: "", lastModeId: null, prePlanModeId: null,
    fast: false, additionalDirectories: [], title: id, createdAt: 1,
    updatedAt: 1, sessionId: null,
    pinned: false, archived: false, sourceChatId: null, kind: "chat", ...over,
  };
}

describe("legacy agent-history migration (Phase 2c)", () => {
  afterEach(() => {
    closeZerosDb();
  ...
  it("imports legacy chats + messages into the engine DB, idempotently", () => {
    ...
    expect(windowChatMessages("legacy1", 100).map((m) => m.msgId)).toEqual(["m1", "m2"]);
  ...
  it("INSERT-if-absent: never clobbers a chat the engine already has", () => {
    ...
    migrateLegacyAgentHistory();
    expect(listChats().find((c) => c.id === "legacy1")!.title).toBe("Newer Engine Title");
  });

  it("no-op when ZEROS_LEGACY_AGENT_DB is unset (web / cloud / standalone)", () => {
    setZerosDbPathForTesting(tmpFile("zeros.db"));
    migrateLegacyAgentHistory();
