// ──────────────────────────────────────────────────────────
// Legacy state.db → zeros.db migration (Phase 0 fold-in) — one-time, engine-side
// ──────────────────────────────────────────────────────────
//
// Workspaces/meta/detach_state used to live in a SEPARATE engine DB at
// ~/.zeros/state.db. Phase 0 folds them into the unified zeros.db (tables created
// by migration 7). This copies the existing rows over once, engine-side, so the
...
export function migrateLegacyStateDb(): void {
  ...
  try {
    ...
    if (hasTable(legacy, "detach_state")) {
      ...
      if (row) {
        // M7: bind ONLY the columns the INSERT names, POSITIONALLY. The old code
