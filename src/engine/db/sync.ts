// ──────────────────────────────────────────────────────────
// Sync primitives — global rev sequence + delete tombstones (Phase 3)
// ──────────────────────────────────────────────────────────
//
...
// single atomic UPDATE … RETURNING (M16) so correctness no longer rests on the
...
export function headRev(): number {
  const db = openZerosDb();
  const row = db.prepare("SELECT next_rev - 1 AS rev FROM sync_meta WHERE id = 0").get() as
    | { rev: number }
    | undefined;
  return row?.rev ?? 0;
}
...
export function clearTombstone(kind: TombstoneKind, id: string): void {
  openZerosDb().prepare("DELETE FROM sync_tombstones WHERE kind = ? AND id = ?").run(kind, id);
}
...
export function tombstonesSince(kind: TombstoneKind, since: number): string[] {
  const rows = openZerosDb()
    .prepare("SELECT id FROM sync_tombstones WHERE kind = ? AND rev > ? ORDER BY rev")
