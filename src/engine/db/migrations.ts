...
/** v3 — the sidebar chat list (Phase 2a). Faithful to the renderer's
 ...
 *  this `chats` table + the chat-keyed messages table added in Phase 2b; they
...
/** v4 — chat transcripts (Phase 2b). The engine persists agent messages here AS
 *  IT STREAMS them (engine-persists-on-emit; it's the source — works even for a
 *  cloud agent whose client is disconnected). One row per coalesced AgentMessage,
 *  keyed by (chat_id, msg_id) so a streaming message upserts a single row as its
 *  chunks arrive (mirrors electron/db.ts agent_messages). `payload` is the
...
/** v5 — full-text search over transcripts (Phase 3). A plain `content` column
...
/** v6 — incremental delta sync (Phase 3, real pull). Until now `rev` was a
...
/** v7 — fold state.db into zeros.db (Phase 0, "one engine DB"). The Phase-0
...
// §3.6 R6 — per-turn usage (tokens/cost, incl. the per-model breakdown) as a
...
/** M6: migration 7 DROPs the idealized v1 `workspaces`/`messages`/`sessions`/
...
export function runMigrations(db: Database.Database): void {
  ...
  // moved onto the engine bridge (src/native/git.ts → workspace/service.ts), so
  ...
  for (const m of pending) {
    const tx = db.transaction(() => {
      if (m.version === 7) backupNonEmptyV1Tables(db); // M6: preserve non-empty v1 tables
