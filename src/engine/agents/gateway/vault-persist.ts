...
// token, mirroring the GitHub-token H4 rule) and NOT the relay bridge:
//
...
// in src/engine/index.ts and the host in electron/sidecar.ts). It holds NO
 ...
 *  which `electron/keychain-accounts.ts` allowlists for the RENDERER's keychain
...
function asSnapshot(v: unknown): VaultSnapshot | null {
  return v && typeof v === "object" && !Array.isArray(v) ? (v as VaultSnapshot) : null;
}
...
export function parseVaultBlob(raw: string | undefined | null): VaultSnapshot | null {
  if (!raw) return null;
  try {
