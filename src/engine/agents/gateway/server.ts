...
//
// Phase 2a: connects WITHOUT auth — an OAuth-protected backend
// 401s and is recorded as `needs-auth`, the gateway still serves whatever
// connected. Phase 2b adds the per-backend OAuthClientProvider (the
// authProvider hook below) so those light up.
//
// SECURITY: bound to 127.0.0.1 only; it will hold tokens (2b) so it must never
// listen on a public interface. Backend URLs are guarded (oauth-url.ts) before
// connection in 2b.
// ──────────────────────────────────────────────────────────
...
import { aggregateTools, type BackendToolSet, type ToolRoute } from "./aggregate";
...
export class McpGateway {
   ...
   *  rotating-refresh AS invalidates the second (§9). Serializing calls per
  ...
  private readonly guardedFetch = (input: string | URL | Request, init?: RequestInit): Promise<Response> => {
  ...
  private async connectBackends(backends: readonly GatewayBackend[]): Promise<void> {
    ...
    for (const b of backends) {
      ...
      if (b.auth === "header") {
        ...
        transport = new StreamableHTTPClientTransport(new URL(b.url), {
          ...
          requestInit: { headers: { ...(b.headers ?? {}), [header.name]: header.value } },
      ...
      try {
        ...
        const needsAuth =
          b.auth !== "header" && (err instanceof UnauthorizedError || /\b401\b|unauthor/i.test(detail));
        statuses.push({
          name: b.name,
          url: b.url,
        ...
        console.warn(`[mcp-gateway] backend "${b.name}" ${needsAuth ? "needs auth" : "failed"}: ${detail}`);
        try {
  ...
  async authorize(backendName: string): Promise<GatewayBackendStatus> {
    ...
    if (this.pendingAuth) throw new Error("another MCP sign-in is already in progress");
    ...
    const timer = setTimeout(
      () => this.pendingAuth?.reject(new Error("sign-in timed out (no browser redirect within 5 min)")),
      5 * 60_000,
    );
    ...
    try {
      try {
        ...
        if (query.get("state") !== provider.expectedState) {
          throw new Error("authorization state mismatch — refusing (possible CSRF)");
        }
  ...
  async beginAuthorize(backendName: string): Promise<{ authorizationUrl: string }> {
    ...
    this.pendingHeadless = { backend: backendName, provider, transport, client };
  ...
  private handleCallback(req: http.IncomingMessage, res: http.ServerResponse): void {
    try {
      const u = new URL(req.url ?? "/", `http://127.0.0.1:${this.callbackPort}`);
  ...
  /** Run `fn` after any in-flight call to the same backend completes (single-
   *  flight, §9). The lock chain swallows results so one failure never poisons
  ...
  private isAllowedRequest(req: http.IncomingMessage): boolean {
    ...
    if (origin !== undefined) {
      ...
      if (o !== `http://127.0.0.1:${this.port}` && o !== `http://localhost:${this.port}`) return false;
    }
    return true;
  }
  ...
  private async handle(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
    try {
      ...
      console.error(`[mcp-gateway] request error:`, err instanceof Error ? err.message : err);
      if (!res.headersSent) {
        res.statusCode = 500;
        res.end();
      }
