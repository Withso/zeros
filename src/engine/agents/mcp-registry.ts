...
// Codex). See docs/mcp-consolidated-architecture-audit-and-test-plan-2026-06-30.md
// (§5 layering/dedup, §12 Phase 0).
 ...
 *
 *  Scope note: this dedups the Zeros registry against ITSELF. De-duping
 *  Zeros servers against the user's *native* (pass-through) config requires
 *  reading those config files — that lands with the Phase 1b scanner. */
 ...
 *  (src/zeros/agent/mcp-secrets.ts). */
...
export function mcpServersFromSettings(
  effective: RawSettingsDoc | { mcp?: { servers?: unknown } } | null | undefined,
): McpServerRegistration[] {
  const raw = (effective as { mcp?: { servers?: unknown } } | null | undefined)?.mcp?.servers;
 ...
 *  fronts the server's tools on localhost. Consumed by the gateway host
 *  (Phase 2b); surfaced here so the partition is stable + testable now. */
...
const REPO_LOCAL_SETTINGS_RELPATH =
  `${REPO_SETTINGS_DIRNAME}/settings.local.toml`;
...
async function readTrustedRepoLocalDoc(
  ...
  if (
    ...
    (cached.trusted ||
      Date.now() - cached.checkedAt < UNTRUSTED_VERDICT_TTL_MS)
  ) {
...
function composeMcpRegistry(
  ...
  for (const layer of MCP_LAYER_PRECEDENCE) {
    ...
    for (const entry of list) {
      ...
      // can't be both).
      if (s.transport === "http" && (s.auth === "oauth" || s.auth === "header")) {
        ...
        gatewayBackends.push({
          ...
          ...(s.auth === "header"
            ...
            : {}),
          ...(s.auth === "oauth" && s.oauth_client_id ? { clientId: s.oauth_client_id } : {}),
          ...(s.disabled_tools && s.disabled_tools.length ? { disabledTools: s.disabled_tools } : {}),
          source: layer,
        });
        continue;
      }
