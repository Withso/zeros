...
//
//   dev       the engine is `bun <repo>/src/cli.ts` (electron/sidecar.ts
...
// electron/sidecar.ts passes the path as ZEROS_CLAUDE_CLI_PATH — the same
...
//      `bun src/cli.ts`, `pnpm serve:engine`, and vitest.
...
//
// Sibling module: `src/engine/agents/claude-binary.ts` resolves a `claude` for
 ...
 *  binary (electron/sidecar.ts → resolveClaudeCliPaths). */
...
export interface ClaudeCliSource {
  ...
  /** Which tier answered. Logged once per engine boot so a field report tells
   *  us WHICH tier won — a "well-known"/"path" hit in a packaged app means the
