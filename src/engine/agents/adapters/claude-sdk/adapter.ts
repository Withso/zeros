 ...
 *  = accept-edits expressed as H9-safe allow rules). Bash/exec tools are
...
function resolveDefaultPermissionMode(cwd: string): ClaudeMode {
  // H9: an in-repo `.claude` file (a cloned/malicious repo) must NOT be able to
...
interface SdkSession {
   ...
   *  together. See docs/unified-question-card-plan-2026-07-02.md. */
...
export class ClaudeSdkAdapter implements AgentAdapter {
  ...
  /** Run a REAL context compaction (§3.5 Task A/C, claude side). The Agent
  ...
  private async emitContextUsage(state: SdkSession): Promise<void> {
    ...
    try {
      ...
      const categories = (usage.categories ?? [])
        ...
        // through rendered the row TWICE (user report 2026-07-12).
  ...
  private async runConsumer(state: SdkSession): Promise<void> {
    ...
    try {
      for await (const msg of q as AsyncIterable<SDKMessage>) {
        ...
        if (m.type === "result") {
          // §3.5 Task C: refresh the REAL context-window reading for the
          ...
          // §3.6 R3 — Options.maxBudgetUsd is accounted per QUERY run, and
  ...
  async setModel(opts: { sessionId: string; model: string }): Promise<void> {
    ...
    // §3.6 R2 — the fallback detector compares against the CURRENT primary;
  ...
  async updateConfig(opts: {
    ...
    // §3.6 R2/R3 — same by-omission contract: envForChat emits the fallback /
    ...
    // §3.6 R2/R3 — fallbackModel and maxBudgetUsd are creation-time Options
  ...
  private canUseTool(state: SdkSession) {
    return (
      ...
      //    posture as H9-safe rules, since the SDK only ever suggests `setMode`
  ...
  private onUserDialog(state: SdkSession) {
    return (
      ...
      // Diagnostic (Phase 0): surfaces the real dialogKind in the user's Mac
  ...
  private settleQuestion(
    ...
    try {
      ...
      // the next engine-window reconcile / reload (field report 2026-07-04 —
      // "ANSWERED badge disappears"); this update makes the record
  ...
  private buildOptions(state: SdkSession): Options {
    ...
    // §3.6 R2 — overload/unavailable fallback. Never fall back to the model
    ...
    // §3.6 R3 — the per-turn USD cap (Settings → Models → Budget). The SDK
    ...
    const options: Options = {
      ...
      // (so "Full Access" kept prompting per action, §3.2 Task 1). Scope it to
      ...
      //
      // canUseTool and bypass are MUTUALLY EXCLUSIVE by SDK contract. As of
      // @anthropic-ai/claude-agent-sdk 0.3.206 (claude-code 2.1.206), building a
      // query with BOTH `canUseTool` and `bypassPermissions` throws
      ...
      // FAILURE" on EVERY send while in Full Access (field report: Zeros Beta
      // 0.0.11, com.zeros.beta engine.log). Older SDKs only warned, so this never
      // bit until the bundled SDK bump. In bypass the SDK auto-approves and never
      ...
      // §3.6 R2/R3 — reliability + budget knobs (see derivations above).
      ...
      // src/cli.ts` where that lookup works. See binary-resolver.ts for the tier
