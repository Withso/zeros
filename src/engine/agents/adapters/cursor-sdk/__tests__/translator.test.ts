...
// (migration A, Phase 1). Pure logic — no SDK / no key.
...
describe("CursorSdkTranslator", () => {
  it("maps assistant text blocks to agent_message_chunk (coalesced id)", () => {
    const { t, out } = capture();
    t.feed({ type: "assistant", message: { role: "assistant", content: [
  ...
  it("opens an in_progress card on tool_call running, then completes it", () => {
    const { t, out } = capture();
    t.feed({ type: "tool_call", call_id: "c1", name: "readToolCall", status: "running", args: { path: "a.ts" } });
    t.feed({ type: "tool_call", call_id: "c1", name: "readToolCall", status: "completed", result: { success: {} } });
    const u = updates(out);
  ...
  it("maps error status to a failed tool card", () => {
    const { t, out } = capture();
    t.feed({ type: "tool_call", call_id: "c2", name: "shellToolCall", status: "running", args: { command: "ls" } });
    t.feed({ type: "tool_call", call_id: "c2", name: "shellToolCall", status: "error", result: { error: "boom" } });
    const u = updates(out);
    expect(u[0]).toMatchObject({ kind: "execute", status: "in_progress" });
    expect(u[1]).toMatchObject({ sessionUpdate: "tool_call_update", status: "failed" });
  });

  it("does not double-open when assistant tool_use precedes the tool_call message", () => {
    const { t, out } = capture();
    t.feed({ type: "assistant", message: { role: "assistant", content: [
      { type: "tool_use", id: "c3", name: "editToolCall", input: { path: "x.ts" } },
    ] } });
    t.feed({ type: "tool_call", call_id: "c3", name: "editToolCall", status: "running", args: { path: "x.ts" } });
  ...
  it("captures the ERROR/EXPIRED status message as errorDetail", () => {
    const a = capture();
    a.t.feed({ type: "status", status: "ERROR", message: "Rate limit exceeded" });
    expect(a.t.sawError).toBe(true);
  ...
  it("ignores user echoes and tolerates unknown message types", () => {
    const { t, out } = capture();
    t.feed({ type: "user", message: { role: "user", content: [{ type: "text", text: "hi" }] } });
  ...
  const childrenOf = (out: SessionNotification[], parentId: string) =>
    updates(out).filter((u) => (u as { parentToolId?: string }).parentToolId === parentId);
  /** The report surfaced as the parent task card's answer (a content-bearing
   *  tool_call_update on the parent toolCallId). */
  const answerOf = (out: SessionNotification[], parentId: string): string | undefined => {
  ...
  const subagentRun = (steps: unknown[], extraValue: Record<string, unknown> = {}) => {
    const { t, out } = capture();
    t.feed({
      type: "tool_call", call_id: "t1", name: "task", status: "running",
      args: { description: "Explore", prompt: "do it", subagentType: { kind: "explore" } },
    });
    t.feed({
      type: "tool_call", call_id: "t1", name: "task", status: "completed",
      result: { status: "success", value: { isBackground: false, backgroundReason: "agentRequest", conversationSteps: steps, ...extraValue } },
    });
    t.flushSubagents();
    return { out, parentId: (updates(out)[0] as { toolCallId: string }).toolCallId };
  };
  ...
  it("lifts a subagent's conversationSteps into parentToolId-tagged children", () => {
    const { out, parentId } = subagentRun([
      ...
      { type: "toolCall", message: { type: "read", args: { path: "a.ts" }, result: { status: "success", value: { content: "l1\nl2", totalLines: 2 } } } },
      { type: "toolCall", message: { type: "shell", args: { command: "ls" }, result: { status: "success", value: { exitCode: 0, signal: "", stdout: "a.ts", stderr: "", executionTime: 5 } } } },
      { type: "toolCall", message: { type: "grep", args: { pattern: "foo" } } },
      { type: "assistantMessage", message: { text: "Here is the summary." } },
    ]);
    const u = updates(out);
    expect(u[0]).toMatchObject({ sessionUpdate: "tool_call", kind: "task", status: "in_progress" });
    ...
    expect(children[1]).toMatchObject({ sessionUpdate: "tool_call", kind: "read" });
    expect(children[2]).toMatchObject({ sessionUpdate: "tool_call", kind: "execute" });
    expect(children[3]).toMatchObject({ sessionUpdate: "tool_call", kind: "search" });
  });
  ...
  it("surfaces the trailing assistant message as the card's answer (not duplicated as a child)", () => {
    ...
    expect(childrenOf(out, parentId).some((c) => c.sessionUpdate === "agent_message_chunk")).toBe(false);
  });
  ...
  it("maps a subagent edit step to an edit card with a diff", () => {
    const { out, parentId } = subagentRun([
      { type: "toolCall", message: { type: "edit", args: { path: "x.ts" }, result: { status: "success", value: { diffString: "@@ -1 +1 @@\n-a\n+b", linesAdded: 1, linesRemoved: 1 } } } },
    ]);
    const child = childrenOf(out, parentId)[0] as { kind: string; rawInput: { path: string; diff: string } };
  ...
  it("does not emit children for the legacy {success} result shape (backward compat)", () => {
    const { out, parentId } = (() => {
      const { t, out } = capture();
      t.feed({ type: "tool_call", call_id: "t2", name: "task", status: "running", args: { description: "x", prompt: "y" } });
      t.feed({ type: "tool_call", call_id: "t2", name: "task", status: "completed", result: { success: { text: "old shape" } } });
      t.flushSubagents();
      return { out, parentId: (updates(out)[0] as { toolCallId: string }).toolCallId };
    })();
  ...
  it("streams subagent tool calls LIVE via pollSubagents, then narration + report at flush", () => {
      ...
      loadSubagentTranscript: () => ({
        steps: [
          ...
          ...Array.from({ length: visibleTools }, (_, i) => ({
            type: "tool" as const, toolKind: "read", title: "Read", status: "completed" as const, rawInput: { path: `f${i}.ts` },
          })),
        ],
        finalText: "# Report",
      }),
    });
    t.feed({ type: "tool_call", call_id: "tt", name: "task", status: "running", args: { description: "x", prompt: "y", agentId: "sub-1" } });
    const parentId = (updates(out)[0] as { toolCallId: string }).toolCallId;
    const toolChildren = () => childrenOf(out, parentId).filter((c) => c.sessionUpdate === "tool_call");
    ...
    t.feed({ type: "tool_call", call_id: "tt", name: "task", status: "completed", result: { status: "success" } });
    t.flushSubagents();
    expect(toolChildren()).toHaveLength(2); // no dup of the live-streamed tools
    expect(childrenOf(out, parentId).filter((c) => c.sessionUpdate === "agent_message_chunk")).toHaveLength(1); // narration now
  ...
  it("defers the read to flushSubagents — nothing until the run ends", () => {
    ...
    const t = new CursorSdkTranslator({
      ...
      loadSubagentTranscript: () => {
        reads++;
        return { steps: [{ type: "tool", toolKind: "read", title: "Read", status: "completed", rawInput: { path: "a.ts" } }], finalText: "" };
      },
    });
    t.feed({ type: "tool_call", call_id: "tt", name: "task", status: "running", args: { description: "x", prompt: "y", agentId: "sub-1" } });
    t.feed({ type: "tool_call", call_id: "tt", name: "task", status: "completed", result: { status: "success" } });
  ...
  it("prefers the on-disk transcript over conversationSteps and keys by the task agentId", () => {
      ...
      loadSubagentTranscript: (agentId) => {
        ...
        return {
          steps: [
            ...
            { type: "tool", toolKind: "read", title: "Read", status: "completed", rawInput: { path: "a.ts" } },
            { type: "tool", toolKind: "search", title: "Grep", status: "completed", rawInput: { pattern: "x" } },
          ],
          finalText: "# Report",
        };
      },
    });
    t.feed({ type: "tool_call", call_id: "tt", name: "task", status: "running", args: { description: "Explore", prompt: "go", agentId: "sub-123" } });
    // The result carries conversationSteps, but the transcript must WIN.
    t.feed({ type: "tool_call", call_id: "tt", name: "task", status: "completed", result: { status: "success", value: { isBackground: false, conversationSteps: [{ type: "toolCall", message: { type: "glob", args: { globPattern: "**/*" } } }] } } });
    t.flushSubagents();
    const parentId = (updates(out)[0] as { toolCallId: string }).toolCallId;
    ...
    expect(children[0]).toMatchObject({ sessionUpdate: "agent_thought_chunk" });
    expect(children[1]).toMatchObject({ sessionUpdate: "tool_call", kind: "read" });
    expect(children[2]).toMatchObject({ sessionUpdate: "tool_call", kind: "search" });
  });
  ...
  it("falls back to conversationSteps when the transcript is unavailable", () => {
    ...
    t.feed({ type: "tool_call", call_id: "tt", name: "task", status: "running", args: { description: "x", prompt: "y", agentId: "sub-9" } });
    t.feed({ type: "tool_call", call_id: "tt", name: "task", status: "completed", result: { status: "success", value: { isBackground: false, conversationSteps: [{ type: "toolCall", message: { type: "read", args: { path: "a.ts" } } }], resultSuffix: "done" } } });
    t.flushSubagents();
    const parentId = (updates(out)[0] as { toolCallId: string }).toolCallId;
    const children = childrenOf(out, parentId);
    expect(children).toHaveLength(1);
    expect(children[0]).toMatchObject({ sessionUpdate: "tool_call", kind: "read" });
  });
  ...
  it("discovers a running subagent by prompt (agentId absent on the running leg) and streams it LIVE", () => {
    ...
    const t = new CursorSdkTranslator({
      ...
      loadSubagentTranscript: (agentId) =>
        agentId === "disc-1"
          ? {
              steps: [{ type: "tool", toolKind: "read", title: "Read", status: "completed", rawInput: { path: "a.ts" } }],
              finalText: "# Report",
            }
          : null,
    ...
    t.feed({ type: "tool_call", call_id: "tt", name: "task", status: "running", args: { description: "Explore", prompt: "Please explore the codebase thoroughly" } });
    const parentId = (updates(out)[0] as { toolCallId: string }).toolCallId;
    const toolChildren = () => childrenOf(out, parentId).filter((c) => c.sessionUpdate === "tool_call");
  ...
  it("discovers + streams LIVE even when the running-leg args carry NO prompt", () => {
    ...
    const t = new CursorSdkTranslator({
      ...
      loadSubagentTranscript: (agentId) =>
        agentId === "by-recency"
          ? { steps: [{ type: "tool", toolKind: "search", title: "Grep", status: "completed", rawInput: { pattern: "x" } }], finalText: "" }
          : null,
      ...
      discoverSubagentAgentId: (prompt) => (prompt === "" ? "by-recency" : null),
    });
    t.feed({ type: "tool_call", call_id: "tt", name: "task", status: "running", args: { description: "Explore" } });
    const parentId = (updates(out)[0] as { toolCallId: string }).toolCallId;
    t.pollSubagents();
    expect(childrenOf(out, parentId).filter((c) => c.sessionUpdate === "tool_call")).toHaveLength(1);
  });
  ...
  it("prefers the result's transcriptPath over agentId-based resolution at flush", () => {
    ...
    t.feed({ type: "tool_call", call_id: "tt", name: "task", status: "running", args: { description: "x", prompt: "y" } });
    t.feed({ type: "tool_call", call_id: "tt", name: "task", status: "completed", result: { status: "success", value: { agentId: "a-1", transcriptPath: "/abs/sub.jsonl", finalMessage: "done" } } });
    t.flushSubagents();
    const parentId = (updates(out)[0] as { toolCallId: string }).toolCallId;
  ...
  it("uses result.value.finalMessage as the report when no transcript/conversationSteps exist", () => {
    ...
    t.feed({ type: "tool_call", call_id: "tt", name: "task", status: "running", args: { description: "x", prompt: "y" } });
    t.feed({ type: "tool_call", call_id: "tt", name: "task", status: "completed", result: { status: "success", value: { agentId: "a-1", finalMessage: "The final report." } } });
    t.flushSubagents();
    const parentId = (updates(out)[0] as { toolCallId: string }).toolCallId;
  ...
  it("re-emits the authoritative transcript in full when a live guess streamed the WRONG file", () => {
    ...
    const wrong: ParsedSubagentTranscript = {
      steps: [{ type: "tool", toolKind: "read", title: "Read", status: "completed", rawInput: { path: "wrong.ts" } }],
      finalText: "",
    };
    const right: ParsedSubagentTranscript = {
      steps: [
        { type: "tool", toolKind: "search", title: "Grep", status: "completed", rawInput: { pattern: "a" } },
        { type: "tool", toolKind: "search", title: "Grep", status: "completed", rawInput: { pattern: "b" } },
      ],
    ...
    const t = new CursorSdkTranslator({
      sessionId: "s1",
      emit: (n) => out.push(n),
      loadSubagentTranscript: (id) => (id === "wrong" ? wrong : id === "right" ? right : null),
      loadSubagentTranscriptByPath: (p) => (p === "/abs/right.jsonl" ? right : null),
      discoverSubagentAgentId: () => "wrong", // recency guesses the WRONG file
    });
    t.feed({ type: "tool_call", call_id: "tt", name: "task", status: "running", args: { description: "x" } });
    const parentId = (updates(out)[0] as { toolCallId: string }).toolCallId;
    const toolChildren = () => childrenOf(out, parentId).filter((c) => c.sessionUpdate === "tool_call");
    ...
    t.feed({ type: "tool_call", call_id: "tt", name: "task", status: "completed", result: { status: "success", value: { agentId: "right", transcriptPath: "/abs/right.jsonl" } } });
    t.flushSubagents();
