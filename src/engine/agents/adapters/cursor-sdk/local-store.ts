...
// the packaged app, whose host runs under Electron 33 / Node 20.18). This module
// is the same fix for the IN-PROCESS path, where the adapter awaits
// `import("@cursor/sdk")` directly — reached when the engine is not bun
// (`pnpm serve:engine` runs `node dist-engine/cli.js`) or when
// ZEROS_CURSOR_IN_PROCESS=1 forces it. package.json's `engines` allows Node
// >= 20, so that path is not a hypothetical: on Node 20 or 21 every new Cursor
// chat failed, with the same causeless TDZ on every attempt after the first.
//
 ...
 *  src/engine/index.ts passes `undefined` when a relay client's cwd falls
