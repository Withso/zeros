...
import type { ContentBlock, SessionNotification, StopReason } from "../../types";
...
type ToolKind =
  ...
  | "subagent" | "task" | "mcp" | "question" | "other";
...
export interface CursorSdkTranslatorOptions {
  ...
  loadSubagentTranscript?: (subagentAgentId: string) => ParsedSubagentTranscript | null;
  ...
  loadSubagentTranscriptByPath?: (path: string) => ParsedSubagentTranscript | null;
...
export class CursorSdkTranslator {
  ...
  feed(raw: unknown): void {
    ...
    switch (msg.type) {
      ...
      case "task":
        ...
        break;
      case "request":
        // Interactive approval request — wired in Phase 3 (permission
        // round-trip). Ignored in Phase 1.
        break;
      default:
        this.onUnknown?.(raw);
    }
  ...
  private onToolCall(msg: SdkMsg): void {
    ...
    if (mapToolKind(name) === "task" && isObj(args) && typeof args.agentId === "string" && args.agentId) {
    ...
    if (msg.status === "running") {
      ...
      if (mapToolKind(name) === "task") {
        ...
        if (toolCallId && !this.subagents.has(callId)) {
          ...
          if (entry && !entry.promptText) entry.promptText = readTaskPrompt(args);
        }
      }
      return;
    }
  ...
  private markSubagentDone(toolCallId: string, callId: string, result: unknown): void {
    ...
    const transcriptPath =
      value && typeof value.transcriptPath === "string" ? value.transcriptPath : undefined;
    const entry = this.subagents.get(callId);
    if (entry) {
      entry.result = result;
      ...
      this.subagents.set(callId, { toolCallId, agentId, transcriptPath, toolsEmitted: 0, result });
    }
  }
  ...
  pollSubagents(): void {
    ...
    if (this.discoverSubagentAgentId) {
      ...
      for (const e of this.subagents.values()) if (e.agentId) claimed.add(e.agentId);
    ...
    for (const entry of this.subagents.values()) {
      ...
      if (entry.streamedAgentId !== undefined && entry.streamedAgentId !== entry.agentId) {
      ...
      if (tools.length > entry.toolsEmitted) {
        for (let i = entry.toolsEmitted; i < tools.length; i++) this.emitStep(entry.toolCallId, tools[i]);
        ...
        this.onLog?.(`[cursor-sdk] live poll: subagent ${entry.agentId} → ${tools.length} tools so far`);
  ...
  private resolveSubagentFor(entry: {
    ...
    transcriptPath?: string;
    result?: unknown;
  }): { steps: NormalizedSubagentStep[]; finalText: string; sourceAgentId?: string } | null {
    if (this.loadSubagentTranscriptByPath && entry.transcriptPath) {
      ...
      if (t && (t.steps.length > 0 || t.finalText))
        return { ...t, sourceAgentId: agentIdFromTranscriptPath(entry.transcriptPath) };
    }
    if (this.loadSubagentTranscript && entry.agentId) {
      const t = this.loadSubagentTranscript(entry.agentId);
      if (t && (t.steps.length > 0 || t.finalText)) return { ...t, sourceAgentId: entry.agentId };
    }
  ...
  private emitSubagentReport(toolCallId: string, text: string): void {
    this.emit({
      ...
      update: {
        sessionUpdate: "tool_call_update",
        toolCallId,
        content: [{ type: "content", content: { type: "text", text } as ContentBlock }],
      },
    });
  }
  ...
  private openToolCard(callId: string, name: string | undefined, input: unknown): void {
...
function extractResultText(result: unknown): string {
  ...
  if (value) {
    const fromValue: unknown[] = [
      value.finalMessage, value.resultSuffix, value.text, value.result,
      value.output, value.message, value.content,
    ];
  ...
  const candidates: unknown[] = [
    success?.text, success?.result, success?.output, success?.message, success?.content,
    result.text, result.result, result.output, result.message, result.content,
  ];
...
function readTaskResultValue(result: unknown): Record<string, unknown> | null {
  ...
  if (isObj(result.result) && isObj(result.result.value)) return result.result.value;
  return null;
}
...
function extractSubagentResult(
  ...
  const raw = value && Array.isArray(value.conversationSteps) ? value.conversationSteps : [];
  ...
  for (let i = 0; i < raw.length; i++) {
    ...
    if (s.type === "assistantMessage") {
      ...
      if (tc) steps.push({ type: "tool", toolKind: tc.kind, title: tc.title, status: tc.status, rawInput: tc.rawInput, ...(tc.content ? { content: tc.content } : {}) });
    }
  }
  let finalText =
    finalIdx >= 0 ? readStepText((raw[finalIdx] as { message?: unknown }).message) : "";
...
function mapConversationToolCall(message: unknown): {
  ...
  const body = (s: string | undefined): Array<{ type: "content"; content: ContentBlock }> | undefined =>
    s && s.trim().length > 0
      ? [{ type: "content", content: { type: "text", text: s } as ContentBlock }]
      : undefined;

  switch (message.type) {
    case "shell": {
      const command = str(args.command);
      const exitCode = value && typeof value.exitCode === "number" ? value.exitCode : null;
      const out = value ? [str(value.stdout), str(value.stderr)].filter(Boolean).join("\n") : "";
      return {
        title: `Running ${truncate(command || "shell command", 60)}`,
        kind: "execute",
        status: errored || (exitCode !== null && exitCode !== 0) ? "failed" : "completed",
        rawInput: { command },
        content: body(out),
      };
    }
    ...
    case "glob":
      return {
        ...
        content: body(value && Array.isArray(value.files) ? value.files.join("\n") : ""),
      };
    case "grep":
      return {
        ...
        kind: "search",
        status: done,
        rawInput: { pattern: str(args.pattern), path: str(args.path), glob: str(args.glob) },
      };
    case "semSearch":
      return {
...
function describeTool(name: string, args: unknown): string {
  ...
  if (/grep/.test(n)) return `Grep ${truncate(String(a.pattern ?? a.query ?? ""), 40)}`;
  if (/glob/.test(n)) return `Searching for ${a.pattern ?? "files"}`;
  if (/^(todo|updatetodos)/i.test(name)) return "Updating plan";
  if (/^task/.test(n)) return `Subagent ${truncate(String(a.description ?? a.prompt ?? ""), 40)}`;
  return name;
}
