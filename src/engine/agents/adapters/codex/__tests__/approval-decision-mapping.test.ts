// §3.2 Task 9 — the approval decision-mapper is the highest-severity safety
...
describe("SAFETY invariant — an unapproved action NEVER becomes a grant", () => {
  ...
  for (const method of RUN_METHODS) {
    for (const resp of NON_APPROVALS) {
      const label = resp.outcome.outcome === "cancelled" ? "cancelled" : resp.outcome.optionId;
...
describe("defaultMethodResponse", () => {
  for (const method of RUN_METHODS) {
    it(`${method} · cancel/decline pass through as {decision}`, () => {
      expect(defaultMethodResponse(method, "cancel")).toEqual({ decision: "cancel" });
      expect(defaultMethodResponse(method, "decline")).toEqual({ decision: "decline" });
    });
  }
