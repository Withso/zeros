...
// `codex exec --json` stream translator (deleted in B5); the
...
export class CodexAppServerTranslator {
  ...
  /** Phase 2.5 — per-turn token usage (tokenUsage.last) for analytics. */
  ...
  /** Phase 2.5 — per-turn token usage for LLM analytics (no cost over
   ...
   *  honest (§3.5 Task B). Wire-verified 2026-07-12 (codex 0.144.1): on a
  ...
  private onTokenUsage(params: unknown): void {
    ...
    // Phase 2.5 — capture THIS turn's usage (tokenUsage.last) for
  ...
  private onItemStarted(params: unknown): void {
    ...
    switch (item.type) {
      ...
      case "contextCompaction": {
        // The two-state compaction row (§3.5 design rev 2). rawInput.trigger
        ...
        // turn's working group (user spec 2026-07-12).
  ...
  private onItemCompleted(params: unknown): void {
    ...
    switch (item.type) {
      ...
      case "contextCompaction": {
        ...
        // "Context compacted" + Done chip (§3.5 design rev 2).
        const toolCallId = this.toolCallIds.get(item.id);
        if (!toolCallId) return;
        this.emit({
          sessionId: this.sessionId,
...
function describeItem(item: ThreadItemUnion): string {
  switch (item.type) {
    ...
    case "contextCompaction":
      // The RUNNING label (§3.5 design rev 2). onItemCompleted relabels the
