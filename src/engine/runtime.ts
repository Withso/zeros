...
// ──────────────────────────────────────────────────────────
//
// The ENGINE side mirror of electron/runtime-mode.ts. Where the Electron
...
//
//   ZEROS_DEV=1            (set by the electron:dev script AND electron/main.ts)
 ...
 *  PLUS base+8/base+9 for the MCP gateway + its OAuth callback (engine/index.ts
...
// feed (electron/updater.ts), and (c) flip channel-gated feature flags.
//
...
// import.meta.env.VITE_ZEROS_CHANNEL (src/zeros/flags.ts) — this one is for the
...
export function channel(): Channel {
  ...
  // on the happy path: electron/main.ts seeds it from the compile-time bake before
