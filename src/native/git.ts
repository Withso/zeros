// ──────────────────────────────────────────────────────────
// Native bindings — roadmap 03a IPC (git + GitHub + detach + cross-tool)
// ──────────────────────────────────────────────────────────
//
...
//
// Engine source-of-truth: src/engine/git/index.ts. The types below
...
export async function workspaceCreate(
  ...
  // "Open in…" IDE list (per spec) — fire-and-forget so create latency
...
// ── Repo init + clone (Phase 1A modals) ─────────────────
...
export async function workspaceInitRepo(
  ...
  // Creating a local project is a desktop-host op — there's no host FS over the
  // relay. The web build hides the entry point (AddRepoMenu), but guard here too
  // so a stray call fails with a clear message instead of the raw nativeInvoke
  // "requires the Mac app" throw.
  if (!isNativeRuntime()) {
    throw new Error(
...
export async function listWorkspaceFiles(
  ...
  // Local main is outside Electron's trusted worktree roots. Web clients and a
  // desktop renderer whose preload has not appeared yet also use the bridge.
...
// ── GitError shape (from src/engine/git/errors.ts) ──────
