...
// (Phase 2c) The per-chat policies + plan IPC wrappers were removed: those
// surfaces now live in renderer localStorage (device-local.ts / policies.ts),
// and the electron/db.ts store backing them is gone.
 ...
 * Cmd+O is the topbar's "open worktree in default app" — see column2-topbar.)
