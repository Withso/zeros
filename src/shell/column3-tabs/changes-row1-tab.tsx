// ──────────────────────────────────────────────────────────
// ChangesRow1Tab — THE pinned row-1 Changes tab
// ──────────────────────────────────────────────────────────
//
...
// this tab, which diverts the open to the shared row-1 File-tab flow so the
...
import { useRow1DirtyEditorIds } from "./code-editor/row1-editor-state";
...
// Memoised like FilesTab: the row-1 body switch re-renders on unrelated store
// changes; context hooks below still see live updates.
export const ChangesRow1Tab = React.memo(function ChangesRow1Tab({
  tab,
  active,
  scope,
}: TabBodyProps) {
...
function ChangesSurface({
  ...
  // viewer scale together when column 3 resizes.
  ...
  const applySelection = useCallback(
    (path: string, intent: OpenFileOpts) => {
      const updates: Partial<Omit<Column3Tab, "id" | "type">> = {
        filePath: path,
      ...
      // use-open-file-in-row1; a dirty tab is only ever re-applied same-path).
  ...
  // shared row-1 flow (focus an existing File tab / spawn a new one) and the
  ...
  const rowActions: RowActions = useMemo(
    () => ({
      ...
      // needs the whole-tree context) — never a turn view. Row 1's targets are
      ...
      {sidebarVisible && (
        <>
          {/* Sidebar — toolbar header + the changed-file list. Percentage
              width keeps the split proportional as column 3 resizes;
