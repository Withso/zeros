...
import { writeWorkspaceFile, type WriteFileResult } from "@/native/files";
import { triggerGitRefresh } from "@/shell/use-git-refresh-key";
import { setRow1EditorDirty } from "./row1-editor-state";
...
export function SourceEditor({
  ...
  const changeDraft = useCallback(
    (next: string) => {
      setDraft(next);
      setRow1EditorDirty(editorId, next !== baselineRef.current);
    },
    [editorId],
  );

  // Mirror the dirty state to the per-tab registry (see row1-editor-state) so
  ...
  useEffect(() => {
    setRow1EditorDirty(editorId, dirty);
    return () => setRow1EditorDirty(editorId, false);
  }, [editorId, dirty]);
  ...
  useEffect(() => {
    ...
    if (sync.kind === "adopt-disk") {
      ...
      setRow1EditorDirty(editorId, false);
    }
