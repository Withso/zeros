...
// the shared shiki worker (src/zeros/agent/renderers/syntax.ts); the
...
import { isGitErrorShape } from "@/native/git";
import { getLang } from "@/zeros/agent/renderers/syntax";
...
import { zerosCodeViewOptions } from "@/zeros/appearance/diff-theme";
...
import { useWorkspaceDispatch } from "@/zeros/store/workspace-store";
import { useOpenFileInRow1, type OpenFileOpts } from "../use-open-file-in-row1";
import type { ViewerMode } from "../column3-tab-manager";
...
interface FileViewerProps {
  /** Owning row-1 tab. Used to close only this tab if an external delete leaves
   * neither a working-tree file nor a reviewable Git diff. */
  tabId: string;
  /** Whether this File tab is the currently visible row-1 tab. */
  active: boolean;
...
export function FileViewer({
  ...
  // Row-1 review controls (Viewed / Discard). Subscribe so the checkbox + the
  ...
  // After marking viewed / discarding, open the next change (D3 directional
  ...
  const advance = useCallback(() => {
    ...
    // the sweep advances ITS selection instead of opening a separate row-1 File
    // tab; standalone File tabs use the preserved identity below.
    if (next)
      (onOpenPath ?? openInRow1)(
        next,
        changeAdvanceIntent({
          diffScope,
          diffSha,
  ...
  const onDiscardConfirm = useCallback(() => {
    ...
    void discardPath(workspaceId, targetPath)
      ...
      .catch((error: unknown) => {
        ...
        // even on error so row 1, All Files, the list, and the badge cannot
    ...
    <div ref={viewerRef} className="bg-bg1 flex h-full min-h-0 flex-col">
      ...
      >
        ...
        {/* Keep a writable editor mounted while Diff/Preview is selected. Its
            draft + dirty registration then survive every row-1 view switch;
...
// ── Diff view (row-1, virtualized @pierre/diffs CodeView) ──────
...
function DiffView({
  ...
  // root. Column3's shared provider supplies the off-main-thread worker pool.
  return (
