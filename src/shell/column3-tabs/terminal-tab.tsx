// ──────────────────────────────────────────────────────────
// TerminalPanel — column-3 row 2 (Setup / Run / Terminal tabs)
// ──────────────────────────────────────────────────────────
//
...
//               actually started (via that button, or the row-2 header Run
...
// auto-seeding survive every row-1 tab switch and row-2 collapse. `expanded`
...
import { type RunAction } from "@zeros/core/run-actions";

import { cn } from "../../zeros/ui/cn";
import { Button } from "../../zeros/ui";
import { Badge, Tooltip } from "../../zeros/ui/primitives";
import { DynamicIcon } from "../../zeros/ui/icon-registry";
...
import { ptyTerminals, onPtyTerminalsChanged } from "../../native/pty";
import {
  workspaceSetupInfo,
  workspaceRunLog,
  type Workspace,
 ...
 *  row-2 panel (see git history) — the terminals live here now, so the
...
function useEngineTerminalSync(folder: string): { synced: boolean } {
  ...
  // Column-2 terminal-AGENT chats spawn engine PTYs (keyed by CHAT id) into the
  // SAME shared registry. Excluding them here keeps a col-2 agent terminal from
...
export function TerminalPanel({
  ...
  // with none — so row 2 always has a shell ready. activate=false preserves the
  ...
  const handleAddTerminal = useCallback(() => {
    // The col-3 terminal panel always spawns plain shells (no agentId) — the
    // "system terminal" surface. (The terminal AGENT default is col-2 chat-only.)
  ...
  // action: the empty-state button, or the row-2 header Run control.
    ...
    >
      ...
      >
        {/* Recent workspace terminals stay mounted (PTY + xterm survival); only
            the active folder/tab shows. The xterm fits/focuses only while row 2
        ...
        {/* Run — the zero-actions "Add run script" state (A1: the affordance
...
/** The Run sub-tab's body when NO run actions are configured (A1): the
...
/** Row 2's header: Setup · action(s) · Terminal(s) · [+], using the same pill
 *  geometry and selected background as row 1. Setup + run actions carry no ✕;
...
/** One row-2 pill. Shared chrome constants keep it identical to row 1 while
      ...
      {onClose && (
        <>
          {/* Fade gradient — masks the trailing edge of the label so the close
              ...
              The tab is bg2 whenever active/hovered, matching row 1. Pure overlay
