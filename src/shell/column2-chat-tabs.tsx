// ──────────────────────────────────────────────────────────
// Column 2 — Chat tabs strip (per-pane, sticky + responsive)
// ──────────────────────────────────────────────────────────
//
// One strip per PANE (split-pane groups live in column2-panes.tsx —
...
// keeper; those moved to Column2Panes). The strip lists the pane's
...
import { useWorkspaceDispatch, type ChatThread } from "../zeros/store/store";
import { useNativeRuntime } from "../native/runtime";
import { OpenInSubmenu } from "./column2-topbar";
 ...
 *  strip in column2-panes.tsx so the chrome band doesn't jump height. */
...
export interface Column2ChatTabsProps {
   ...
   *  col-3 expand button lives here while the panel is collapsed —
   *  its old home, the col-2 topbar, is hidden). */
...
export function Column2ChatTabs({
  ...
  // Gate the "Open in" submenu the same way the (hidden) topbar gates
  // its split button — hidden until the preload bridge lands (and on the
  // web build, which has no apps to open into), and only when this pane
  // resolved a workspace folder to target.
