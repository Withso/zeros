// ──────────────────────────────────────────────────────────
// Column 2 — Top Bar (Roadmap 03a UI Phase 1B)
// ──────────────────────────────────────────────────────────
//
// Mounts above the existing chat-tabs row in Column 2. Layout:
//
//   ┌────────────────────────────────────────────────────────┐
...
//      end of the row-1 Changes tab's PR row as the labelled "Target branch"
//      pill; see TargetBranchButton in src/shell/pr/target-branch-select.tsx.)
//
//   Right:
//     - "Open in" split button — app logo (click = open the worktree in
//       the current default app, ⌘O) + chevron (menu: Finder first, the
//       IDEs detected on this Mac — Cursor / VS Code / Antigravity / Zed /
//       Xcode / Devin / opencode — then Terminal, then Copy path ⌘C).
//       Picking anything except Copy path re-points the default + logo.
//       Detection state lives in src/native/open-apps.ts.
//     - Reserved space (parent Column 2 still owns "Show Panel" when
//       col3 is collapsed; we don't render it here, parent does)
//
...
import { selectActiveFolder, useWorkspaceStore } from "../zeros/store/store";
import { type Project } from "../zeros/store/projects-store";
import { findWorkspaceForFolder } from "../zeros/store/workspace-resolution";
import { LOCAL_MAIN_LABEL } from "../zeros/store/local-main-workspace";
import {
...
// transparent (flat on the column's bg1), matching col-1 / col-3 headers, and
// borderless (no chrome seam in the chat column). h-10 keeps this secondary
// workspace toolbar aligned with Column 3 below the global navigation bar.
...
function useResolvedContext(folder: string | null): ResolvedContext {
  // Resolve from the SAME project list the sidebar uses — bridge-sourced on the
  // web build (loadProjects() is empty there), localStorage on desktop. Reactive,
  // so a freshly-created worktree / web bridge (re)connect re-resolves without
  // the "No workspace selected" flash.
...
export function OpenAppIcon({ app }: { app: DetectedOpenApp }) {
  ...
  // opencode + cursor ship bundled SVG marks (src/assets/agents/) — reuse
  // them for the CLI (no .app bundle to pull an icon from) / icon-miss case.
 ...
 *  (see Column2ChatTabs). The ⌘O / ⌘C shortcuts stay owned by the hidden
...
export interface Column2TopBarProps {
  /** Whether Column 3 (the right design panel) is currently collapsed.
   *  When true, the col-3 toggle button moves from Column 3's header
   *  (which is unmounted) into this topbar so the user can still
   *  expand the design panel from here. */
  col3Collapsed?: boolean;
  /** Toggles the design-panel visibility. Wired here so the col-3
   *  collapse button can live as a real DOM descendant of this drag
   *  region (Superset pattern — sibling overlays don't subtract). */
  onToggleCol3?: () => void;
}

export function Column2TopBar({
  col3Collapsed = false,
  onToggleCol3,
}: Column2TopBarProps = {}) {
  ...
  if (!project) {
    return (
      <div ref={topbarRef} className={TOPBAR_CLS}>
        ...
        {col3Collapsed && onToggleCol3 && (
          <Col3ToggleButton col3Collapsed onToggle={onToggleCol3} />
        )}
      </div>
    );
  }
  ...
  return (
    <div ref={topbarRef} className={TOPBAR_CLS}>
      ...
      {/* The Run affordance lives at the right end of Column 3's terminal row
          — see RunControl (run-control.tsx). Create PR moved too: it lives in
          the row-1 Changes tab's PR status row (ChangesRow1Tab → PrStatusRow),
