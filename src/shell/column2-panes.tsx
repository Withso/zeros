// ──────────────────────────────────────────────────────────
// Column 2 — Split-pane container (VS Code-style chat groups)
// ──────────────────────────────────────────────────────────
//
...
// model lives in src/zeros/store/chat-panes.ts; this file is only the
...
//   - Dragging a tab over a pane shows Cursor-style drop zones:
...
//     displayed chat the global active chat (composer hotkeys, col-3
...
// in Column2ChatTabs (which is now a per-pane dumb strip): resolving
...
import {
  ...
  spawnNewChatTab,
} from "../zeros/store/spawn-default-chat";
...
interface PaneCtx {
  ...
  /** Pane hosting workspace-level top-right chrome (col-3 expand). */
...
export function Column2Panes({
  ...
  // ── Selection keeper (2026-07-06 invariant, moved here) ──
  // A workspace in view must ALWAYS have an active chat — Column 2
  // renders a dead pane for a null selection. See the original comment
  // block in column2-chat-tabs (pre-panes) for the full history.
  useEffect(() => {
    if (
  ...
  const handleCloseTab = useCallback(
    (paneId: string, chat: ChatThread, e?: React.MouseEvent) => {
      ...
      // column2-chat-close for the exact "never used" contract. Draft inputs:
...
function ChatPane({ paneId, ctx }: { paneId: string; ctx: PaneCtx }) {
  ...
  // selection (composer hotkeys, col-3 scope and the breadcrumb all follow).
  ...
  return (
    ...
    >
      <Column2ChatTabs
        ...
        extraTrailing={
          // With the col-2 topbar hidden, this strip's right end is the
          // only remaining on-screen home for the col-3 expand button —
          // hosted by the TOP-RIGHT pane so it hugs the panel it opens.
          ctx.col3Collapsed &&
          ctx.onToggleCol3 &&
          paneId === ctx.topRightPaneId ? (
            <Col3ToggleButton col3Collapsed onToggle={ctx.onToggleCol3} />
          ) : undefined
        }
      />
