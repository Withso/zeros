...
// rowless target (see src/engine/git/setup-runner.ts), so the trunk runs
...
/** The fixed row-2 tab id for the Setup view (a run tab uses the run
 *  session's deterministic id; everything else is a plain session id). Lives
 *  here (not terminal-tab.tsx) so column3's create-time initialization can share it
