...
describe("selectPanelTerminals", () => {
  it("excludes column-2 terminal-agent (chat-bound) PTYs — the phantom-panel-tab bug", () => {
    // A real panel shell AND a column-2 terminal-agent PTY (keyed by chat id)
...
describe("selectExcludedChatTerminalIds", () => {
  it("excludes terminal chats whether LIVE or ARCHIVED — closes the kill-race", () => {
    // A just-closed col-2 terminal agent is archived synchronously but its PTY is
    // reaped async. Keeping the archived id excluded is what stops the col-3
