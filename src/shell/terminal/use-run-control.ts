...
// ──────────────────────────────────────────────────────────
//
// Shared by row 2's terminal panel (its per-action Run tabs) and the
// row-2 header split-button. Resolves the repo's `[[scripts.run_actions]]`
// (with the legacy single `scripts.run` migrated to one default "run" action
// at read time — see @zeros/core/run-actions), filters them to this OS, and
...
import { useResolvedSettings } from "../../zeros/settings/use-settings";
import { workspaceStartRun, workspaceStopRun } from "../../native/git";
