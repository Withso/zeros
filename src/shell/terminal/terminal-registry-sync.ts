...
// registry into row 2's terminal tab strip (pure helpers)
// ──────────────────────────────────────────────────────────
//
...
import type { PtyTerminalLike } from "../../zeros/bridge/pty-bridge";
...
// mirrors normalizePath in zeros/store/workspace-resolution.ts.
export function normalizeFolder(p: string): string {
  return p.replace(/^\/private(\/(?:var|tmp|etc)\/)/, "$1");
}

/** The CHAT ids whose PTYs belong to Column-2 terminal AGENTS (the engine keys
 *  those PTYs by chat id) and must therefore be EXCLUDED from the Column-3
 *  panel's adopt-list — see selectPanelTerminals.
 *
 *  Crucially this includes ARCHIVED terminal chats. Closing a col-2 terminal
 ...
 *  excluded, and adopt it as a phantom "Terminal" tab in column 3 (the reported
...
/** Partition the engine's shared terminals into what the COL-3 PANEL should
 *  adopt.
 *
 *  `excludedSessionIds` are Column-2 terminal-AGENT PTYs (keyed by CHAT id) that
 *  live in the SAME engine registry but are surfaced via the chat list, NOT this
 *  panel. Excluding them is what stops a new Column-2 terminal agent from popping
 *  a phantom tab in the Column-3 panel — and excluding them from the alive-set
