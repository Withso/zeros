...
//
// Column 2's grow factor reads --zeros-column-2-ratio. The variable used to be
...
function bootValue(): string {
  return declared.get(COLUMN_2_RATIO_VAR) ?? "";
}
...
describe("applyBootLayoutVars", () => {
  ...
  it("publishes the persisted ratio on <html>", () => {
    store.set(COLUMN_2_RATIO_KEY, "0.35");
    applyBootLayoutVars();
  ...
  it("agrees with what the column hook reads a moment later", () => {
    ...
    store.set(COLUMN_2_LEGACY_WIDTH_KEY, "640");
    applyBootLayoutVars();
    expect(bootValue()).toBe(String(readPersistedColumn2Ratio()));
  });
});
