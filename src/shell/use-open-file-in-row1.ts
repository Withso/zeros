// ──────────────────────────────────────────────────────────
// useOpenFileInRow1 / useOpenChatFileInRow1 — open a file in the row-1 viewer
// ──────────────────────────────────────────────────────────
//
// useOpenFileInRow1 (All Files / Changes) follows the ACTIVE tab: an active
...
// a fresh tab opens (see planRow1Open). useOpenChatFileInRow1
...
/** Reduce an absolute path to one relative to `cwd` so the row-1 viewer reads
...
/** Open a row-1 file, optionally in Diff mode. `opts.diff` opens the viewer's
 ...
 *  the open tab's mode in place rather than going dead on the first one (D2). */
/** Diff intent carried onto the row-1 tab: open in Diff mode, which diff to show
...
export function useOpenFileInRow1(): (
  path: string,
  opts?: OpenFileOpts,
) => void {
  ...
  return useCallback(
    (rawPath: string, opts?: OpenFileOpts) => {
      ...
      // a different filter switches its diff + discard affordance in place (D2).
      const intent = {
      ...
      // the editor and silently destroy the draft. (planRow1Open only reuses a
...
/** Open a file referenced in agent-chat output in row 1 — focusing it if it's
...
export function useOpenChatFileInRow1(): (
