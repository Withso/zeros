...
// the chat's folder and agent. Recent views live in Column2Panes' bounded
...
//                                 Column2TerminalDeck sibling so the
...
import {
  useAgentSessions,
  useChatSession,
} from "../zeros/agent/sessions-hooks";
...
import {
  ...
  pickDefaultAgent,
  useDefaultAgent,
} from "../zeros/panels/default-agent";
import { useAgentsSnapshot } from "../zeros/agent/agents-cache";
import { isRemovedAgent } from "../zeros/agent/agent-runnable";
...
import { finishPreparedColumn2ChatView } from "./column2-chat-intent";
...
export function Column2ChatView({
  ...
  // as useChatCwd / column1 / the topbar. Without the fallback, a chat with
  ...
  if (!active) {
    // 2026-06-18: EmptyComposer (the no-chat "start a new chat" landing) was
    // deleted per the user's redesign and no longer exists in this worktree.
  ...
  // Column2TerminalDeck (a sibling of this component in
  // Column2Workspace) so they survive chat-tab switches. This
...
function ChatBody({
  ...
  // Column 3's longer presentation-only settling window is deliberately not a
  // session gate.
  useEffect(() => {
    if (!surfaceActive && !pendingAutoSend) {
      ...
      if (preparing) {
        void session
          .hydrateChat()
          .finally(() => finishPreparedColumn2ChatView(chatId));
      }
      return;
    }
    ...
    void (async () => {
      ...
      // Phase 2 chat overhaul: pass cwd verbatim so the gateway can surface
