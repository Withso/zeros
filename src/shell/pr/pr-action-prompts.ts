...
// directly in the engine — see PrIslandActionBehavior). The wording is the
// user-approved 2026-07-19 spec verbatim, with the base branch (and remote,
// where named) interpolated.
// ──────────────────────────────────────────────────────────
...
export interface PrActionContext {
  /** The PR base / target branch (e.g. `main`) — the only piece of workspace
   *  state the 2026-07-19 prompt wording interpolates. */
  baseBranch: string;
 ...
 *  archive act directly in the engine (2026-07-19), and show-checks is pure
 *  navigation. Wording is the user-approved 2026-07-19 spec. */
...
export function buildReviewCommentContext(args: {
  ...
  const verdict = state && state !== "COMMENTED" ? ` (${state.replace(/_/g, " ").toLowerCase()})` : "";
  const quoted = body
    .split("\n")
    .map((l) => `> ${l}`)
    .join("\n");
