...
export interface PrInstructionInputs {
  ...
  remote?: string;
  /** GitHub owner/repository resolved from the configured remote. Pinning the
   *  CLI target prevents `gh` choosing origin in a multi-remote/fork setup. */
  repository?: string;
  /** Total uncommitted paths = staged + unstaged + untracked + conflicted. */
  uncommittedCount: number | null;
  /** False when gitStatus failed. Omitted means known for older callers. */
  statusKnown?: boolean;
...
function uncommittedSentence(count: number | null): string {
  if (count == null) return "The uncommitted change count could not be read.";
...
function blockerLines(input: PrInstructionInputs): string[] {
  const lines: string[] = [];
  if (input.statusKnown === false) {
    lines.push(
      "Git status could not be read. Inspect the worktree for unresolved conflicts or an in-progress merge, rebase, cherry-pick, or revert before committing.",
    );
  }
...
/** POSIX-shell single quoting, including embedded apostrophes. Branches and
 * remotes are repository data, not trusted shell source. */
...
export function buildPrInstructions(input: PrInstructionInputs): string {
  ...
  const upstreamLine =
    hasUpstream == null
      ? "The upstream branch could not be determined."
      : hasUpstream
  ...
  const createCmd = [
    "gh pr create",
    ...(input.repository ? ["--repo", shellQuote(input.repository)] : []),
    ...(draft ? ["--draft"] : []),
    "--base",
