...
Desktop sign-in opens `${APP_ORIGIN}/auth/desktop`, a branded Zeros provider
page served by Cloudflare Pages. The page sends only the selected provider,
state, and PKCE challenge through Pages to Railway. Railway uses the Desktop
Application client ID and sends the browser directly through WorkOS to Google
or GitHub. The return page is again on `APP_ORIGIN`, then opens the exact
installed channel. No WorkOS API key or refresh token enters Pages, the browser
page, a deep link, or the packaged application.
...
After the hosted-callback change is merged to `main`, activate Alpha in this
order:

1. In the WorkOS Alpha environment, open `Zeros Desktop Alpha` and add
   `https://app-alpha.zeros.build/auth/desktop/callback`. Confirm Google and
   GitHub are enabled. Keep the legacy custom-scheme and loopback redirects only
   for rollback.
...
3. In Railway's `alpha` environment, set `AUTH_PROVIDER=workos`, exact
   ...
   signing secret. For the qualified standard WorkOS contract,
   `AUTH_ISSUER=https://api.workos.com/user_management/<web-client-id>` and
   `AUTH_JWKS_URL=https://api.workos.com/sso/jwks/<web-client-id>`. Remove
   `AUTH_BROKER_SECRET` and leave `ZEROS_SELF_HOSTED` unset for official Alpha.
