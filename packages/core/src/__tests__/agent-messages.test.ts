...
// The shared coalescer (Phase 2b): both the renderer (live) and the engine
// (persist-on-emit) fold streaming chunks into AgentMessages with applyUpdate.
...
function userChunk(text: string, messageId?: string): SessionNotification {
  return {
    sessionId: "s",
    update: { sessionUpdate: "user_message_chunk", content: { type: "text", text }, messageId },
  } as unknown as SessionNotification;
}
...
describe("applyUpdate — shared agent-message coalescer", () => {
  ...
  it("folds tool_call + tool_call_update onto a single tool message", () => {
    ...
    const tool = msgs.find((m) => m.kind === "tool") as AgentToolMessage | undefined;
...
describe("applyUpdate — question tool-call identity + stamp durability", () => {
  ...
  it("preserves the zerosQuestion stamp when a tool_result replaces rawOutput", () => {
    ...
    msgs = msgs.map((m) =>
      m.kind === "tool"
        ? { ...m, rawOutput: { zerosQuestion: { outcome: "answered", summary: "Redux" } } }
        : m,
    );
    ...
    const out = (msgs[0] as AgentToolMessage).rawOutput as Record<string, unknown>;
    expect(out.vendor).toBe("result body");
    expect((out.zerosQuestion as { outcome?: string }).outcome).toBe("answered");
  });
...
describe("applyUpdate — error_notice rows", () => {
  ...
  it("ignores a notice with no id or message (defensive)", () => {
    const out = applyUpdate([], {
      sessionId: "s",
      update: { sessionUpdate: "error_notice", noticeId: "", severity: "warning", message: "" },
    } as unknown as SessionNotification);
