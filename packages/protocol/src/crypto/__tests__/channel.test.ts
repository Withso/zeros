...
function makePipe(): [HandshakeIO, HandshakeIO] {
  ...
  const clientIO: HandshakeIO = {
    send: (f) => c2s.push(f),
    recv: () => s2c.pull(),
  };
  const serverIO: HandshakeIO = {
    send: (f) => s2c.push(f),
    recv: () => c2s.pull(),
  };
...
describe("EncryptedChannel", () => {
  ...
  // The client may sign with a non-extractable key through an asynchronous
  ...
  it("rejects a MITM: host identity not matching the pinned offer key", async () => {
    ...
    if (clientResult.status === "rejected") {
      ...
      expect((clientResult.reason as HandshakeError).code).toBe(
        "bad-signature",
      );
    }
  });
  ...
  it("rejects an untrusted / revoked device", async () => {
    ...
    if (serverResult.status === "rejected") {
      expect((serverResult.reason as HandshakeError).code).toBe(
        "untrusted-device",
      );
    }
  });
