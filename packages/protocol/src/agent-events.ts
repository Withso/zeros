...
//   - engine (apps/desktop/src/engine/agents/**) emits SessionNotification
//     payloads from each adapter's translator
//   - renderer (apps/desktop/src/renderer/features/agent/**) folds them into the chat UI
//
 ...
 *
 *  `web_search`, `mcp`, `subagent`, and `question` let the
...
export type ToolKind =
  ...
  | "background_task"
  // Context compaction — a first-class two-state row ("Compacting.."
  // → "Context compacted · Done"). Codex streams it from the
  // contextCompaction item lifecycle; Claude emits a settled row from
  // compact_boundary.
  | "compaction"
  // The turn continued on a fallback model (primary overloaded /
  ...
  | "model_switch"
  // The per-turn budget cap ended the turn cleanly. Renders as the
...
export interface ToolCall {
  ...
  /** Set by adapters when this tool was invoked from
...
// existing `@zeros/protocol/agent-events` + `../bridge/agent-events` import
...
/** Subagent entries surfaced by the custom-agent picker.
...
export interface AgentThoughtChunkUpdate {
  ...
  /** See the same field on AgentMessageChunkUpdate. */
  parentToolId?: string | null;
}
...
// adapter reshapes it into the provider's native answer format.
...
export type StopReason =
  ...
  | "refusal"
  | "cancelled"
  // Distinct terminal endings, mapped from the Claude SDK's
  ...
  // cap (recoverable via Continue); blocking_limit = the provider's
