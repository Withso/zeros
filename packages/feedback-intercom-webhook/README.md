...
the conversation body (zero Intercom setup required), and *also* a real Intercom
...
```bash
cd packages/feedback-intercom-webhook
...
npx wrangler@4 secret put AUTH_AUDIENCE      # the Auth0 API identifier — same value as the backend's
...
secret let you file feedback as *any* address, which the Worker attached to a
...
- **Standalone:** no workspace runtime deps beyond `@zeros/core/scrub`; it is
  deployed on its own and is not wired into the app's build or release.
