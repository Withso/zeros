...
// every click/mousedown/mouseup on the renderer. That means a column
// header that's both a drag handle AND a popover-trigger row can't
// have single-click-outside dismissal — the OS hit-tests the drag
// region first and the renderer never sees the press
// (electron/electron#1354, status:wontfix). Cursor and friends solve
// this by removing the CSS drag region entirely and reimplementing
// drag in JS via IPC. That's what these commands enable.
//
// Flow per drag:
