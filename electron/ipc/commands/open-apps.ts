...
// ──────────────────────────────────────────────────────────
//
//   detect_open_apps — probe this Mac for the IDEs the topbar "Open in"
//                      menu can list (Cursor, VS Code, Antigravity, Zed,
//                      Xcode, Devin, opencode CLI) plus the always-present
//                      Finder/Terminal, returning each with its REAL app
...
//
// Security posture (matches shell.ts H1/H2): the renderer only ever sends
...
const APP_SEARCH_DIRS = ["/Applications", path.join(os.homedir(), "Applications")];
...
async function probeApp(spec: OpenAppSpec): Promise<LaunchTarget | null> {
  ...
  if (spec.bundleIds && spec.bundleIds.length > 0 && process.platform === "darwin") {
 ...
 *
 *  Deliberately NOT Electron's `app.getFileIcon`: on macOS 26 (Tahoe) with
 *  Electron 33 / Chrome 130, getFileIcon hits a fatal Chromium `NOTREACHED`
 *  (SIGTRAP) for ANY path — an UNCATCHABLE native crash that takes the whole
 *  app down the instant detection runs (see the crash-loop this feature
 *  caused). `sips`/`plutil` are plain macOS CLIs (no Chromium), so they can
...
/** CLI launch (opencode): a real Terminal window cd'd into the worktree
 *  running the detected binary by absolute path — same osascript route as
