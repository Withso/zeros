...
// ──────────────────────────────────────────────────────────
//
// Stage 9 §2.9.5. Walks the cwd → parents → home directory looking
...
/** Truncated head of the file as a single string. ~200 chars per
 *  the roadmap, with newlines preserved so the popover can render
 *  the first paragraph cleanly. Binary files (rare for .md) get an
 *  empty preview rather than a garbled byte string. */
function previewFile(filePath: string): { preview: string; size: number; mtime: number } | null {
  let stat: fs.Stats;
  try {
...
/** The set of context-file names each agent loads. Mirrors §2.9.5
 *  of the roadmap. The renderer's agent-ui-registry knows the same
...
function userGlobalRulesPath(agentId: string): string | null {
  ...
  if (id.startsWith("codex") || id.includes("openai")) return path.join(home, ".codex", "AGENTS.md");
  return null;
}
...
export const agentContextFiles: CommandHandler = (args) => {
  ...
  const collect = (
    ...
    for (const p of paths) {
      ...
      if (!meta) continue;
      const scope = p.startsWith(home) && !p.startsWith(cwdResolved + path.sep)
        ? "user"
