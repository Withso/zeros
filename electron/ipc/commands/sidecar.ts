...
export const getEngineToken: CommandHandler = () => {
  // C1: the per-launch secret the renderer presents on its loopback /ws
...
async function respawnAtPath(root: string): Promise<ProjectChangedPayload> {
  ...
  // H5: re-rooting the engine is a privileged action. Apply the SAME guards the
  // cold-start path uses (electron/sidecar.ts defaultProjectRoot) so an attacker
  ...
  if (!isPlausibleProject(root)) {
    throw new Error(`not a project folder (no .git/package.json/.zeros/…): ${root}`);
  }
 ...
 *  by Phase 1A dialogs (Quick Start, Open GitHub project) where the
