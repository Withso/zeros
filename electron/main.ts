...
// engine data dir (com.zeros.dev.<slug>, resolved in src/engine/db/paths.ts),
...
const INSTANCE_LABEL = INSTANCE_SLUG
  ? INSTANCE_SLUG.replace(/[-._]+/g, " ").replace(/\b\w/g, (c) => c.toUpperCase())
  : "";
...
// data dirs purely from env (src/engine/runtime.ts isDevRuntime()); seed it so
  ...
  const legacyBase =
    ch === "dev"
      ...
      : ch === "beta"
        ...
        : "design.zeros.app";
  const legacyDirName =
    ch === "dev" && INSTANCE_SLUG ? `${legacyBase}.${INSTANCE_SLUG}` : legacyBase;
...
function setupLogFile(): void {
  ...
  const record = (level: "log" | "info" | "warn" | "error", args: unknown[]) => {
  ...
  process.on("uncaughtException", (err) => {
    ...
    // stack only — no user content. See src/zeros/analytics/boot.tsx.
...
// (website/marketing, etc.) inherit. If those were running first, a
...
function createMainWindow(): BrowserWindow {
  ...
  const win = new BrowserWindow({
    ...
    ...(restoredPosition ?? {}),
    // minWidth tuned so all three columns stay visible at the floor:
    //   col 1 (workspace): 248 px fixed
    //   col 2: 320 px hard min (see column2-workspace.tsx)
    //   col 3: 200 px hard min (see column3.tsx)
    ...
    // the window's left edge: col-1, col-2's topbar (when col-1 is hidden),
    // and the Settings header. All of those are a full-bleed 40px h-10 top
    ...
    // glyphs beside them (2026-07-12 feedback: y=14 read ~2px low next to
    // the col-1 toggle). x=19 keeps them clear of the window's left edge.
  ...
  // Roadmap 03b iframe migration — strip X-Frame-Options and CSP
  // frame-ancestors so browser-tab iframes can load arbitrary URLs.
  // See electron/iframe-headers.ts for the rationale.
  installIframeHeaderStripping(win.webContents.session);

  // M13: deny sensitive permissions (camera/mic/geolocation/clipboard-read) to
  ...
  const isAppOrigin = (url: string | undefined): boolean =>
    !url || url === "null" || url.startsWith("file://") || url.startsWith(DEV_URL);
  ...
  if (isDev) {
    ...
    win.webContents.on(
      ...
      (_event, errorCode, errorDescription, validatedURL, isMainFrame) => {
        // Recover only the top-level Mac-app document. Column-3 browser
    ...
    void win.webContents
      ...
      .then((value: unknown) => {
        ...
        const message =
          ...
          `website/marketing) running on the configured port. ` +
...
app.whenReady().then(async () => {
  ...
  // Roadmap 03b iframe migration — column-3 browser tabs are
  ...
  // Stage 2: element picker — install/uninstall via
...
app.on("before-quit", () => {
  shutdownSidecar();
  // Interactive PTYs are now engine-owned (src/native/pty.ts → engine bridge),
  ...
  // (Phase 2c) The legacy agent-history SQLite handle (electron/db.ts) is gone —
