...
// prod sign-in to Beta. See schemeForChannel() in src/engine/runtime.ts.
//
// Resolved LAZILY (a function, not a module-const): electron/main.ts seeds
...
async function handleUrl(rawUrl: string): Promise<void> {
  ...
  if (action === "open") {
    ...
    try {
      ...
      // H5: a web page can fire `zeros://open?path=/` with one click. Apply the
