...
// ──────────────────────────────────────────────────────────
//
// The generic keychain_* IPC bridge (electron/ipc/commands/secrets.ts) lets the
...
// electron/ipc/commands/auth-session.ts — which the renderer never reads
 ...
 *  in src/native/secrets.ts (the renderer source of truth; mirrored here because
...
export function isRendererKeychainAccount(account: string): boolean {
  ...
  // prefix treatment as the per-agent keys above. See src/zeros/agent/mcp-secrets.ts.
  ...
  // into the agent's process env at spawn. See src/zeros/agent/env-vault.ts.
  ...
  // vault migration (src/zeros/settings/migrate-legacy.ts,
