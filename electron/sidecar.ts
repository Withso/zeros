...
// src/engine/runtime.ts and gives Stable, Beta, Dev, and per-worktree overrides
 ...
 *  (src/engine/pty/pty-host.cjs): the ABI-matching node-pty module and the
...
function resolvePtyHostPaths(): {
  ...
  if (IS_PACKAGED) {
    ...
    const repoRoot = path.resolve(__dirname, "..");
    const dev = path.join(repoRoot, "src", "engine", "pty", "pty-host.cjs");
 ...
 *  is `bun <repo>/src/cli.ts`, works perfectly. That asymmetry is why this shipped
 *  (field report: 0.0.14 Beta + Production).
 *
...
async function reapOrphanEngines(skipPid?: number): Promise<void> {
  ...
  if (IS_DEV && instance) {
    const cliPath = path.join(path.resolve(__dirname, ".."), "src", "cli.ts");
...
async function doSpawnEngine(
  ...
  // can run the one-time Phase 2c migration into the unified Zeros DB. The engine
  ...
  // stdin EOF or a ppid change away from this pid (engine/index.ts
  ...
  // Account-binding config for the engine. src/engine/auth/verify-jwt.ts is a
  // generic RS256/ES256/HS256 JWT verifier with no provider dependency: it was
  // written against Supabase and runs unchanged against Auth0, which is why
  // only the VALUES below changed in that migration. (The vars were once named
  // SUPABASE_JWT_*, which implied a dependency that never existed.)
  // Forwarded so the engine can verify a REMOTE
  ...
  // L5: pin the issuer in PROD (defense in depth atop the tenant-scoped JWKS +
  ...
  // C1: per-launch secret the engine requires on its loopback /ws upgrade. The
  ...
  // token vault here for safeStorage persistence (P0-1). DELIBERATELY not routed
...
/** Per-launch loopback /ws auth token (C1). Minted once per app launch, handed
 ...
 *  events on src/engine/** within a few hundred ms. The previous
...
export function startEngineCodeWatcher(): void {
  ...
  const shouldIgnoreEventPath = (filename: string | null): boolean => {
    ...
    // `.env` under src/engine) must still trigger a respawn.
...
export function startWatchdog(): void {
  ...
  state.watchdogTimer = setInterval(async () => {
    ...
    try {
      ...
      // matching (the new engine is `bun src/cli.ts` or the prod
