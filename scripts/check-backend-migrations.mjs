#!/usr/bin/env node
// ──────────────────────────────────────────────────────────
// check-backend-migrations — control-plane migration forward-only guard
// ──────────────────────────────────────────────────────────
//
// One of the two migration ladders in this repo (`check:migrations` covers the
// engine's SQLite ladder). backend/migrations/ has the higher blast radius:
// it owns the control plane's Postgres — every team, membership, invitation,
// and audit row.
//
// The specific failure this prevents: `runMigrations` (backend/src/migrate.ts)
...
//
// That runner also runs at service BOOT (backend/src/index.ts awaits
...
// Run: `pnpm check:backend-migrations`. Exit 0 = ok, 1 = violation.
// ──────────────────────────────────────────────────────────
...
try {
  mainFiles = execFileSync(
    "git",
    ["ls-tree", "--name-only", "origin/main", `${DIR}/`],
  ...
  mainFiles = null; // origin/main not fetched
}
...
if (mainFiles) {
  ...
  for (const f of mainFiles) {
    ...
    try {
      if (gitShow(`origin/main:${DIR}/${f}`) !== readFileSync(`${DIR}/${f}`, "utf8")) {
        errs.push(
  ...
  console.log(
    "ℹ check:backend-migrations — origin/main unavailable; ran naming + sequence only (in CI use actions/checkout with fetch-depth: 0).",
  );
}

if (errs.length > 0) {
  console.error("✖ check:backend-migrations — violation(s):");
  for (const e of errs) console.error(`  • ${e}`);
  process.exit(1);
}
console.log(
  `✓ check:backend-migrations — ${named.length} migration(s), naming + sequence${mainFiles ? " + forward-only vs origin/main" : ""} OK`,
);
