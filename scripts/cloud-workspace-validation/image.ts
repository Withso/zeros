// ──────────────────────────────────────────────────────────
// zeros-engine-v0 — the Daytona cloud-validation image build spec
// ──────────────────────────────────────────────────────────
//
...
// `bun` is also baked so the runtime can be switched via start-engine.sh.
//
// What's baked:           Why:
//  • node:22-bookworm      real Node (engine + PTY host + Cursor host; bun can't run those)
//  • bun                   optional alternate engine runtime
//  • git, openssh, curl     git-clone seeding + Mutagen's scp/ssh agent injection
//  • python3/make/g++       node-gyp, to compile the native bindings from source
//  • Mutagen + agents       file-mirror validation without rebuilding the image
//  • the 3 agent SDKs       brought in by the repo's own pinned `pnpm install`
...
/** Pin Mutagen to a known-good release. The harness currently uses the official
 *  linux-amd64 binary; redistribution terms require separate review before this
 *  image becomes a product artifact. */
  ...
  return (
    Image.base(NODE_BASE_IMAGE)
      ...
      .env({
        ...
        ZEROS_PTY_HOST_SCRIPT: `${SANDBOX_REPO_DIR}/apps/desktop/src/engine/pty/pty-host.cjs`,
        ZEROS_CURSOR_HOST_SCRIPT: `${SANDBOX_REPO_DIR}/apps/desktop/src/engine/agents/adapters/cursor-sdk/host/cursor-host.cjs`,
      ...
      .runCommands(
        ...
        // 3. Mutagen + its agent bundle for file-mirror validation. Best-effort:
        //    the core bridge checks do not need it, so a release-URL hiccup
        ...
        `cd ${SANDBOX_REPO_DIR} && pnpm rebuild better-sqlite3`,
      ...
      .runCommands(
        "chmod +x /usr/local/bin/start-engine.sh /usr/local/bin/egress-probe.sh",
      )
