...
export async function runDesignGitMenuSmoke({ page, check }) {
  ...
  })).toEqual({ background: "rgba(0, 0, 0, 0.5)", backdropFilter: "none" });
  const title = dialog.getByRole("heading", { name: "Review Design changes", exact: true });
  await expect(title).toHaveCSS("font-size", "15px");
  await expect(dialog.locator('[data-slot="dialog-header"]')).toHaveCSS("padding", "12px 16px 0px");
  await expect(dialog.locator('[data-slot="dialog-body"]')).toHaveCSS("padding", "24px 16px");
  await expect(dialog.locator('[data-slot="dialog-footer"]')).toHaveCSS("padding", "10px");
  ...
  expect(titleBox.x + titleBox.width).toBeLessThan(closeBox.x);
  await expect(dialog.locator(".design-review-footer")).toHaveCSS("border-top-width", "1px");
  check("Design review shares the 15px heading, top-right close, footer divider and 50% overlay", true);
