...
/** A fake network: every provider answers as a correctly scoped Alpha key. */
function network(overrides: Record<string, Reply> = {}, railwayEnvironment = "alpha") {
  const seen: string[] = [];
  const replies: Array<[RegExp, Reply]> = [
    [/user\/tokens\/verify/, { status: 200, body: { result: { status: "active", expires_on: "2026-12-24T00:00:00Z" } } }],
    [/pages\/projects/, { status: 200, body: { success: true, result: [] } }],
    [/r2\.cloudflarestorage\.com\/zeros-cloud-workspaces-alpha\?/, { status: 200 }],
    [/r2\.cloudflarestorage\.com\/zeros-cloud-workspaces-(beta|production)\?/, { status: 403 }],
    [/databases\/zeros-control-plane-alpha$/, { status: 200, body: {} }],
    [/databases\/zeros-control-plane-(beta|production)$/, { status: 404 }],
    ...
    [/api\.workos\.com/, { status: 200, body: { data: [] } }],
    ...
    [/environments\/alpha\/secrets/, { status: 200, body: { secrets: [] } }],
  ];
  const fetch = async (url: string, init: RequestInit = {}) => {
    seen.push(url);
    let reply: Reply | undefined;
    for (const [pattern, value] of Object.entries(overrides)) if (new RegExp(pattern).test(url)) reply = value;
    if (!reply && url.includes("backboard.railway.com")) {
    ...
    reply ??= replies.find(([pattern]) => pattern.test(url))?.[1] ?? { status: 599 };
...
describe("agent credential check", () => {
  ...
  it("warns when a key also reaches Beta or Production", async () => {
    const { deps } = network({
      "databases/zeros-control-plane-production$": { status: 200, body: {} },
      "zeros-cloud-workspaces-beta\\?": { status: 200 },
    }, "production");
  ...
  it("flags an admin Boat key, a soon-expiring token and a rejected key", async () => {
    const { deps } = network({
      ...
      "user/tokens/verify": { status: 200, body: { result: { status: "active", expires_on: "2026-09-30T00:00:00Z" } } },
      "api\\.workos\\.com": { status: 401 },
    });
    const results = byId(await runProviderChecks(VALUES, deps));
  ...
  it("skips unset providers without calling them", async () => {
    ...
    expect(seen.some((url) => url.includes("boat.dev") || url.includes("workos.com"))).toBe(false);
  });
