// ──────────────────────────────────────────────────────────
// provision.ts — create a spike sandbox + start the engine.
// ──────────────────────────────────────────────────────────
//
//   pnpm tsx scripts/cloud-spike/provision.ts
//
...
// .context/cloud-spike/state.json for the load-test scripts.
//
...
async function waitForHealth(url: string, previewToken: string, tries = 40): Promise<boolean> {
  for (let i = 0; i < tries; i++) {
    try {
      const res = await fetch(url, { headers: { "x-daytona-preview-token": previewToken } });
      if (res.ok) {
...
async function main() {
  ...
  const sandbox = await daytona.create(
    {
      ...
      // Engine owns sleep (§5); the native auto-stop MUST be disabled — a running
      // agent is a background process and does NOT reset Daytona's timer (§2.1).
      autoStopInterval: 0,
      ...
      labels: { app: "zeros", role: "engine-spike" },
    },
    { timeout: 180 },
  );

  console.log(`  ✓ sandbox ${sandbox.id} (${(sandbox as { state?: string }).state ?? "started"})`);
  ...
  // Preview coords. The raw link's token rotates on restart; the signed URL
  // carries its token in the query (max 24 h) for a cleaner reconnect.
  const link = await sandbox.getPreviewLink(ENGINE_CLOUD_PORT);
  let signedUrl: string | undefined;
  try {
    const signed = await sandbox.getSignedPreviewUrl(ENGINE_CLOUD_PORT, 86_400);
    signedUrl = signed.url;
  } catch {
    /* signed URLs optional for the spike */
  }
  ...
  if (healthy) {
    ...
    console.log(`      daytona ssh ${sandbox.id}  # then: tail -f /home/daytona/engine.log`);
  }
  console.log("");
  console.log(`  preview:  ${link.url}`);
  console.log(`  signed:   ${signedUrl ?? "(unavailable)"}`);
  console.log(`  port:     ${ENGINE_CLOUD_PORT}`);
  console.log("");
  console.log(`  Next:  pnpm tsx scripts/cloud-spike/test-client.ts   # the Phase 1 exit criterion`);
  console.log(`         pnpm tsx scripts/cloud-spike/egress.ts        # §14 #4 (Cursor egress)`);
  console.log(`         pnpm tsx scripts/cloud-spike/lifecycle.ts     # §14 #2 (resume latency)`);
  console.log(`         pnpm tsx scripts/cloud-spike/soak-wss.ts      # §14 #1 (WSS soak)`);
  console.log("");
}
