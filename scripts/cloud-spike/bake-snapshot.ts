...
// ──────────────────────────────────────────────────────────
//
//   pnpm tsx scripts/cloud-spike/bake-snapshot.ts
//
...
import { makeDaytona, SNAPSHOT_NAME, RESOURCES, DAYTONA_TARGET } from "./config";
...
async function main() {
  ...
  console.log(`  resources: ${RESOURCES.cpu} vCPU / ${RESOURCES.memory} GiB / ${RESOURCES.disk} GiB disk`);
  console.log(`  (this clones + builds the engine in-box — expect several minutes)\n`);

  const started = Date.now();
  await daytona.snapshot.create(
    { name: SNAPSHOT_NAME, image, resources: RESOURCES },
    { onLogs: (line: string) => process.stdout.write(line.endsWith("\n") ? line : line + "\n") },
  );

  const mins = ((Date.now() - started) / 60000).toFixed(1);
  console.log(`\n  ✓ Snapshot "${SNAPSHOT_NAME}" registered in ${mins} min.`);
  console.log(`    Next: pnpm tsx scripts/cloud-spike/provision.ts\n`);
}
