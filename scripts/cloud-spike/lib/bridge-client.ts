// ──────────────────────────────────────────────────────────
// A minimal Node client for the Zeros bridge protocol (spike).
// ──────────────────────────────────────────────────────────
//
...
// for the load-tests. The envelope (id/timestamp) is hand-rolled; PROTOCOL_VERSION
// is pinned to the engine's (the engine rejects a mismatch loudly, so drift can't
// pass silently). Uses the `ws` package (already a repo dep).
// ──────────────────────────────────────────────────────────

import { WebSocket } from "ws";
import { randomUUID } from "node:crypto";

/** Keep in lockstep with packages/core/src/version.ts (engine rejects a skew). */
...
export interface BridgeClientOpts {
  /** ws(s)://…/ws?token=<zerosCloudToken> (built by config.bridgeWsUrl). */
  url: string;
  /** Daytona preview-proxy token → header `x-daytona-preview-token`. Omit when
   *  dialing a signed preview URL (token already in the query). */
  previewToken?: string;
...
export class BridgeClient {
  ...
  private readonly ptyExit: Array<(sessionId: string, code: number | null) => void> = [];
  private readyResolve: ((info: { root: string; version: string }) => void) | null = null;
  ...
  connect(): Promise<{ root: string; version: string }> {
    return new Promise((resolve, reject) => {
      ...
      const ws = new WebSocket(
        this.opts.url,
        this.opts.previewToken
          ? { headers: { "x-daytona-preview-token": this.opts.previewToken } }
          : undefined,
      );
      this.ws = ws;

      const openTimer = setTimeout(() => reject(new Error("ws open timed out (10s)")), 10_000);
      ws.on("open", () => clearTimeout(openTimer));
      ws.on("error", (err) => reject(err instanceof Error ? err : new Error(String(err))));
      ws.on("close", (code, reason) => this.failAll(`socket closed ${code} ${reason}`));
      ws.on("message", (data) => {
        let msg: AnyMsg;
        try {
  ...
  private dispatch(msg: AnyMsg): void {
    switch (msg.type) {
      case "ENGINE_READY": {
        ...
        this.readyResolve?.({ root: this.engineRoot, version: this.engineVersion });
        this.readyResolve = null;
        break;
      }
      case "CONNECTION_REJECTED":
        this.failAll(`engine rejected the connection: ${String(msg.reason)} — ${String(msg.message ?? "")}`);
        break;
      ...
      case "PTY_DATA":
        for (const cb of this.ptyData) cb(String(msg.sessionId), String(msg.data));
        break;
      case "PTY_EXIT":
        for (const cb of this.ptyExit)
          cb(String(msg.sessionId), msg.exitCode === null ? null : Number(msg.exitCode));
        break;
      default:
        break;
    }
  ...
  request(op: string, params: Record<string, unknown> = {}): Promise<unknown> {
    return new Promise((resolve, reject) => {
      const id = this.send({ type: "WORKSPACE_REQUEST", source: "client", op, params });
  ...
  ptyCreate(args: {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.pending.set(id, { resolve: (m) => resolve(m as AnyMsg), reject, timer });
    });
  }
